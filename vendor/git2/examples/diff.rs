...
use git2::{
	Blob, Diff, DiffOptions, Error, Object, ObjectType, Oid,
	Repository,
};
use git2::{
	DiffDelta, DiffFindOptions, DiffFormat, DiffHunk, DiffLine,
};
use std::str;
...
fn run(args: &Args) -> Result<(), Error> {
	...
	if let Some("diff-index") =
		args.flag_format.as_ref().map(|s| &s[..])
	{
		opts.id_abbrev(40);
	}
	...
	let head =
		tree_to_treeish(&repo, Some(&"HEAD".to_string()))?.unwrap();
	let mut diff = match (t1, t2, args.cache()) {
		(Some(t1), Some(t2), _) => repo.diff_tree_to_tree(
			t1.as_tree(),
			t2.as_tree(),
			Some(&mut opts),
		)?,
		...
		(Some(t1), None, _) => repo.diff_tree_to_workdir_with_index(
			t1.as_tree(),
			Some(&mut opts),
		)?,
		(None, None, _) => {
	...
	if args.flag_patch || !stats {
		diff.print(args.diff_format(), |d, h, l| {
			print_diff_line(d, h, l, args)
		})?;
		if args.color() {
			print!("{}", RESET);
		}
