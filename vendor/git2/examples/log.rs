...
use git2::{
	Commit, DiffOptions, ObjectType, Repository, Signature, Time,
};
...
fn run(args: &Args) -> Result<(), Error> {
	...
	for commit in &args.arg_commit {
		...
		if revspec.mode().contains(git2::RevparseMode::SINGLE) {
			...
			if revspec.mode().contains(git2::RevparseMode::MERGE_BASE)
			{
				...
				let o =
					repo.find_object(base, Some(ObjectType::Commit))?;
				revwalk.push(o.id())?;
			}
			revwalk.hide(from)?;
		}
	...
	let (mut diffopts, mut diffopts2) =
...
fn print_commit(commit: &Commit) {
	...
	for line in
		String::from_utf8_lossy(commit.message_bytes()).lines()
	{
		println!("    {}", line);
	}
	println!();
...
fn match_with_parent(
	...
	let b = commit.tree()?;
	let diff =
		repo.diff_tree_to_tree(Some(&a), Some(&b), Some(opts))?;
...
impl Args {
	fn min_parents(&self) -> usize {
		...
		self.flag_min_parents.unwrap_or(if self.flag_merges {
			2
		} else {
			0
		})
	...
	fn max_parents(&self) -> Option<usize> {
		...
		self.flag_max_parents.or(if self.flag_no_merges {
			Some(1)
		} else {
			None
		})
