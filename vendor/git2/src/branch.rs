...
impl<'repo> Branch<'repo> {
	...
	pub fn name_is_valid(name: &str) -> Result<bool, Error> {
		...
		unsafe {
			try_call!(raw::git_branch_name_is_valid(
				&mut valid,
				name.as_ptr()
			));
		}
	...
	pub fn upstream(&self) -> Result<Branch<'repo>, Error> {
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_branch_upstream(
				&mut ret,
				&*self.get().raw()
			));
			Ok(Branch::wrap(Binding::from_raw(ret)))
...
impl<'repo> Iterator for Branches<'repo> {
	...
	fn next(
		&mut self,
	) -> Option<Result<(Branch<'repo>, BranchType), Error>> {
		let mut ret = ptr::null_mut();
		let mut typ = raw::GIT_BRANCH_LOCAL;
		unsafe {
			try_call_iter!(raw::git_branch_next(
				&mut ret, &mut typ, self.raw
			));
			let typ = match typ {
