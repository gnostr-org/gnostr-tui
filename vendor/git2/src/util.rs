...
pub fn iter2cstrs_paths<T, I>(
	...
	(Vec<CString>, Vec<*const c_char>, raw::git_strarray),
	Error,
>
where
	T: IntoCString,
...
pub fn path_to_repo_path(path: &Path) -> Result<CString, Error> {
	...
	match path.components().next() {
		...
		Some(Component::RootDir) => {
			err!("repo path `{}` should be relative", path)
		}
		Some(Component::CurDir) => {
			err!("repo path `{}` should not start with `.`", path)
		}
	...
	{
		match path.to_str() {
			None => return Err(Error::from_str(
				"only valid unicode paths are accepted on windows",
			)),
...
pub fn cstring_to_repo_path<T: IntoCString>(
	path: T,
...
#[cfg(windows)]
fn fixup_windows_path<P: Into<Vec<u8>>>(
	path: P,
) -> Result<CString, Error> {
...
mod tests {
	...
	fn path_to_repo_path_no_weird() {
		...
		assert_err!(
			"./foo",
			"repo path `./foo` should not start with `.`"
		);
		assert_err!(
			"../foo",
			"repo path `../foo` should not start with `..`"
