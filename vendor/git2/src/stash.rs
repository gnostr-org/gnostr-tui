...
use crate::{
	panic, raw, IntoCString, Oid, Signature, StashApplyProgress,
	StashFlags,
};
...
impl<'a> StashSaveOptions<'a> {
	...
	pub unsafe fn raw(
		&mut self,
	) -> *const raw::git_stash_save_options {
		self.raw_opts.flags =
			self.flags.unwrap_or_else(StashFlags::empty).bits
				as c_uint;
		self.raw_opts.message = crate::call::convert(&self.message);
		self.raw_opts.paths.count =
			self.pathspec_ptrs.len() as size_t;
		self.raw_opts.paths.strings =
...
pub type StashApplyProgressCb<'a> =
	dyn FnMut(StashApplyProgress) -> bool + 'a;
...
impl<'cb> StashApplyOptions<'cb> {
	...
	pub fn new() -> StashApplyOptions<'cb> {
		...
		assert_eq!(
			unsafe {
				raw::git_stash_apply_init_options(
					&mut opts.raw_opts,
					1,
				)
			},
	...
	{
		self.progress = Some(
			Box::new(callback) as Box<StashApplyProgressCb<'cb>>
		);
...
fn convert_progress(
	...
	match progress {
		raw::GIT_STASH_APPLY_PROGRESS_NONE => {
			StashApplyProgress::None
		}
		raw::GIT_STASH_APPLY_PROGRESS_LOADING_STASH => {
			StashApplyProgress::LoadingStash
		}
		raw::GIT_STASH_APPLY_PROGRESS_ANALYZE_INDEX => {
			StashApplyProgress::AnalyzeIndex
		}
		raw::GIT_STASH_APPLY_PROGRESS_ANALYZE_MODIFIED => {
			StashApplyProgress::AnalyzeModified
		}
		raw::GIT_STASH_APPLY_PROGRESS_ANALYZE_UNTRACKED => {
			StashApplyProgress::AnalyzeUntracked
		}
		raw::GIT_STASH_APPLY_PROGRESS_CHECKOUT_UNTRACKED => {
			StashApplyProgress::CheckoutUntracked
		}
		raw::GIT_STASH_APPLY_PROGRESS_CHECKOUT_MODIFIED => {
			StashApplyProgress::CheckoutModified
		}
		raw::GIT_STASH_APPLY_PROGRESS_DONE => {
...
extern "C" fn stash_apply_progress_cb(
	...
	panic::wrap(|| unsafe {
		let mut options =
			&mut *(payload as *mut StashApplyOptions<'_>);
		let res = {
