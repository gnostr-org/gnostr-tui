...
impl<'cb> RebaseOptions<'cb> {
	...
	pub fn new() -> RebaseOptions<'cb> {
		...
		assert_eq!(
			unsafe { raw::git_rebase_init_options(&mut opts.raw, 1) },
			0
		);
		opts
	}
	...
	pub fn rewrite_notes_ref(
		...
		self.rewrite_notes_ref =
			Some(CString::new(rewrite_notes_ref).unwrap());
		self
	}
...
impl<'repo> Rebase<'repo> {
	...
	pub fn orig_head_name(&self) -> Option<&str> {
		let name_bytes = unsafe {
			crate::opt_bytes(
				self,
				raw::git_rebase_orig_head_name(self.raw),
			)
		};
	...
	pub fn orig_head_id(&self) -> Option<Oid> {
		unsafe {
			Oid::from_raw_opt(raw::git_rebase_orig_head_id(self.raw))
		}
	}
	...
	pub fn inmemory_index(&mut self) -> Result<Index, Error> {
		...
		unsafe {
			try_call!(raw::git_rebase_inmemory_index(
				&mut idx, self.raw
			));
...
impl<'rebase> Iterator for Rebase<'rebase> {
	...
	fn next(
		&mut self,
	) -> Option<Result<RebaseOperation<'rebase>, Error>> {
...
impl RebaseOperationType {
	...
	pub fn from_raw(
		raw: raw::git_rebase_operation_t,
	) -> Option<RebaseOperationType> {
		match raw {
			raw::GIT_REBASE_OPERATION_PICK => {
				Some(RebaseOperationType::Pick)
			}
			raw::GIT_REBASE_OPERATION_REWORD => {
				Some(RebaseOperationType::Reword)
			}
			raw::GIT_REBASE_OPERATION_EDIT => {
				Some(RebaseOperationType::Edit)
			}
			raw::GIT_REBASE_OPERATION_SQUASH => {
				Some(RebaseOperationType::Squash)
			}
			raw::GIT_REBASE_OPERATION_FIXUP => {
				Some(RebaseOperationType::Fixup)
			}
			raw::GIT_REBASE_OPERATION_EXEC => {
...
impl<'rebase> RebaseOperation<'rebase> {
	...
	pub fn exec(&self) -> Option<&str> {
		unsafe {
			str::from_utf8(
				crate::opt_bytes(self, (*self.raw).exec).unwrap(),
			)
			.ok()
		}
	}
