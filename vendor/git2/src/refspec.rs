...
impl<'remote> Refspec<'remote> {
	...
	pub fn dst_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_refspec_dst(self.raw))
				.unwrap()
		}
	}
	...
	pub fn dst_matches(&self, refname: &str) -> bool {
		...
		unsafe {
			raw::git_refspec_dst_matches(self.raw, refname.as_ptr())
				== 1
		}
	}
	...
	pub fn src_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_refspec_src(self.raw))
				.unwrap()
		}
	}
	...
	pub fn src_matches(&self, refname: &str) -> bool {
		let refname = CString::new(refname).unwrap();
		unsafe {
			raw::git_refspec_src_matches(self.raw, refname.as_ptr())
				== 1
		}
	}
	...
	pub fn bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_refspec_string(self.raw))
				.unwrap()
		}
	}
