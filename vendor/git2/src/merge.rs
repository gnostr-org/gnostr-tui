...
impl<'repo> AnnotatedCommit<'repo> {
	...
	pub fn id(&self) -> Oid {
		unsafe {
			Binding::from_raw(raw::git_annotated_commit_id(self.raw))
		}
	}
	...
	pub fn refname_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_annotated_commit_ref(&*self.raw),
			)
			.unwrap()
		}
	}
...
impl MergeOptions {
	...
	pub fn new() -> MergeOptions {
		...
		assert_eq!(
			unsafe { raw::git_merge_init_options(&mut opts.raw, 1) },
			0
		);
		opts
	}
	...
	pub fn standard_style(
		&mut self,
		standard: bool,
	) -> &mut MergeOptions {
		self.file_flag(
			raw::GIT_MERGE_FILE_STYLE_MERGE as u32,
			standard,
		)
	}
	...
	pub fn simplify_alnum(
		&mut self,
		simplify: bool,
	) -> &mut MergeOptions {
		self.file_flag(
			raw::GIT_MERGE_FILE_SIMPLIFY_ALNUM as u32,
			simplify,
		)
	}
	...
	pub fn ignore_whitespace(
		...
		self.file_flag(
			raw::GIT_MERGE_FILE_IGNORE_WHITESPACE as u32,
			ignore,
		)
	}
	...
	pub fn ignore_whitespace_change(
		...
		self.file_flag(
			raw::GIT_MERGE_FILE_IGNORE_WHITESPACE_CHANGE as u32,
			ignore,
		)
	}
	...
	pub fn ignore_whitespace_eol(
		...
		self.file_flag(
			raw::GIT_MERGE_FILE_IGNORE_WHITESPACE_EOL as u32,
			ignore,
		)
	}
	...
	pub fn patience(&mut self, patience: bool) -> &mut MergeOptions {
		self.file_flag(
			raw::GIT_MERGE_FILE_DIFF_PATIENCE as u32,
			patience,
		)
	}
	...
	pub fn minimal(&mut self, minimal: bool) -> &mut MergeOptions {
		self.file_flag(
			raw::GIT_MERGE_FILE_DIFF_MINIMAL as u32,
			minimal,
		)
	}
