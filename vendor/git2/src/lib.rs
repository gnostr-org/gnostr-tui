...
pub use crate::index::{
	Index, IndexConflict, IndexConflicts, IndexEntries, IndexEntry,
	IndexMatchedPath,
};
...
pub use crate::message::{
	message_prettify, message_trailers_bytes, message_trailers_strs,
	MessageTrailersBytes, MessageTrailersBytesIterator,
	MessageTrailersStrs, MessageTrailersStrsIterator,
...
pub use crate::remote::{
	FetchOptions, PushOptions, Refspecs, Remote, RemoteConnection,
	RemoteHead, RemoteRedirect,
};
pub use crate::remote_callbacks::{
...
pub use crate::worktree::{
	Worktree, WorktreeAddOptions, WorktreeLockStatus,
...
impl IndexEntryExtendedFlag {
	...
	is_bit_set!(
		is_skip_worktree,
		IndexEntryExtendedFlag::SKIP_WORKTREE
	);
...
impl ObjectType {
	...
	pub fn str(&self) -> &'static str {
		unsafe {
			let ptr =
				call!(raw::git_object_type2string(*self)) as *const _;
	...
	pub fn from_str(s: &str) -> Option<ObjectType> {
		let raw = unsafe {
			call!(raw::git_object_string2type(
...
impl ConfigLevel {
	...
	pub fn from_raw(raw: raw::git_config_level_t) -> ConfigLevel {
		match raw {
			raw::GIT_CONFIG_LEVEL_PROGRAMDATA => {
...
impl SubmoduleIgnore {
	...
	pub fn from_raw(raw: raw::git_submodule_ignore_t) -> Self {
		match raw {
			...
			raw::GIT_SUBMODULE_IGNORE_UNTRACKED => {
...
impl SubmoduleUpdate {
	...
	pub fn from_raw(raw: raw::git_submodule_update_t) -> Self {
		match raw {
			raw::GIT_SUBMODULE_UPDATE_CHECKOUT => {
				SubmoduleUpdate::Checkout
			}
			raw::GIT_SUBMODULE_UPDATE_REBASE => {
			...
			raw::GIT_SUBMODULE_UPDATE_DEFAULT => {
...
impl From<FileMode> for i32 {
	fn from(mode: FileMode) -> i32 {
		match mode {
			FileMode::Unreadable => {
				raw::GIT_FILEMODE_UNREADABLE as i32
			}
			FileMode::Tree => raw::GIT_FILEMODE_TREE as i32,
			FileMode::Blob => raw::GIT_FILEMODE_BLOB as i32,
			FileMode::BlobGroupWritable => {
				raw::GIT_FILEMODE_BLOB_GROUP_WRITABLE as i32
			}
			FileMode::BlobExecutable => {
				raw::GIT_FILEMODE_BLOB_EXECUTABLE as i32
			}
...
impl From<FileMode> for u32 {
	fn from(mode: FileMode) -> u32 {
		match mode {
			FileMode::Unreadable => {
				raw::GIT_FILEMODE_UNREADABLE as u32
			}
			FileMode::Tree => raw::GIT_FILEMODE_TREE as u32,
			FileMode::Blob => raw::GIT_FILEMODE_BLOB as u32,
			FileMode::BlobGroupWritable => {
				raw::GIT_FILEMODE_BLOB_GROUP_WRITABLE as u32
			}
			FileMode::BlobExecutable => {
				raw::GIT_FILEMODE_BLOB_EXECUTABLE as u32
			}
