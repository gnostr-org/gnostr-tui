...
use crate::{
	raw, Blob, Buf, Commit, Error, ObjectType, Oid, Repository, Tag,
	Tree,
};
...
impl<'repo> Object<'repo> {
	...
	pub fn short_id(&self) -> Result<Buf, Error> {
		unsafe {
			let buf = Buf::new();
			try_call!(raw::git_object_short_id(
				buf.raw(),
				&*self.raw()
			));
			Ok(buf)
	...
	pub fn describe(
		...
		unsafe {
			try_call!(raw::git_describe_commit(
				&mut ret,
				self.raw,
				opts.raw()
			));
