...
impl<'repo> Describe<'repo> {
	...
	pub fn format(
		...
		let buf = Buf::new();
		let opts =
			opts.map(|o| &o.raw as *const _).unwrap_or(ptr::null());
		unsafe {
			try_call!(raw::git_describe_format(
				buf.raw(),
				self.raw,
				opts
			));
...
impl Binding for DescribeOptions {
	...
	unsafe fn from_raw(
		_raw: *mut raw::git_describe_options,
	) -> DescribeOptions {
		panic!("unimplemened")
	}
...
mod tests {
	...
	fn smoke() {
		...
		let d = t!(repo.describe(
			DescribeOptions::new().show_commit_oid_as_fallback(true)
		));
