...
use crate::{
	call, raw, signature, Error, Object, ObjectType, Oid, Signature,
};
...
impl<'repo> Tag<'repo> {
	...
	pub fn is_valid_name(tag_name: &str) -> bool {
		...
		unsafe {
			call::c_try(raw::git_tag_name_is_valid(
				&mut valid,
				tag_name.as_ptr(),
			))
			.unwrap();
	...
	pub fn message_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(self, raw::git_tag_message(&*self.raw))
		}
	}
	...
	pub fn name_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_tag_name(&*self.raw))
				.unwrap()
		}
	}
	...
	pub fn target_id(&self) -> Oid {
		unsafe {
			Binding::from_raw(raw::git_tag_target_id(&*self.raw))
		}
	}
	...
	pub fn target_type(&self) -> Option<ObjectType> {
		unsafe {
			ObjectType::from_raw(raw::git_tag_target_type(&*self.raw))
		}
	}
