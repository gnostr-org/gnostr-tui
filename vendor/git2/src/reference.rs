...
use crate::{
	call, raw, Blob, Commit, Error, Object, ObjectType, Oid,
	ReferenceFormat, ReferenceType, Repository, Tag, Tree,
};
...
impl<'repo> Reference<'repo> {
	...
	pub fn kind(&self) -> Option<ReferenceType> {
		ReferenceType::from_raw(unsafe {
			raw::git_reference_type(&*self.raw)
		})
	}
	...
	pub fn name_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_reference_name(&*self.raw),
			)
			.unwrap()
		}
	}
	...
	pub fn shorthand_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_reference_shorthand(&*self.raw),
			)
			.unwrap()
		}
	}
	...
	pub fn target(&self) -> Option<Oid> {
		unsafe {
			Binding::from_raw_opt(raw::git_reference_target(
				&*self.raw,
			))
		}
	}
	...
	pub fn target_peel(&self) -> Option<Oid> {
		unsafe {
			Binding::from_raw_opt(raw::git_reference_target_peel(
				&*self.raw,
			))
		}
	}
	...
	pub fn symbolic_target_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_reference_symbolic_target(&*self.raw),
			)
		}
	}
	...
	pub fn resolve(&self) -> Result<Reference<'repo>, Error> {
		...
		unsafe {
			try_call!(raw::git_reference_resolve(
				&mut raw, &*self.raw
			));
	...
	pub fn peel(
		...
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_reference_peel(
				&mut raw, self.raw, kind
			));
			Ok(Binding::from_raw(raw))
		}
	}
...
impl<'repo> Ord for Reference<'repo> {
	fn cmp(&self, other: &Reference<'repo>) -> Ordering {
		c_cmp_to_ordering(unsafe {
			raw::git_reference_cmp(&*self.raw, &*other.raw)
		})
	}
}
...
impl<'repo> Iterator for References<'repo> {
	...
	fn next(&mut self) -> Option<Result<Reference<'repo>, Error>> {
		...
		unsafe {
			try_call_iter!(raw::git_reference_next(
				&mut out, self.raw
			));
...
impl<'repo, 'references> Iterator
	for ReferenceNames<'repo, 'references>
{
	type Item = Result<&'references str, Error>;
	fn next(&mut self) -> Option<Result<&'references str, Error>> {
		let mut out = ptr::null();
		unsafe {
			try_call_iter!(raw::git_reference_next_name(
				&mut out,
				self.inner.raw
			));
...
mod tests {
	...
	fn smoke() {
		...
		assert!(
			head == repo.find_reference("refs/heads/main").unwrap()
		);
		assert_eq!(
...

		{
			...
			assert!(
				repo.references().unwrap().next().unwrap().unwrap()
					== head
			);
			let mut names = repo.references().unwrap();
			let mut names = names.names();
			assert_eq!(
				names.next().unwrap().unwrap(),
				"refs/heads/main"
			);
			assert!(names.next().is_none());
			assert!(
				repo.references_glob("foo").unwrap().count() == 0
			);
			assert!(
				repo.references_glob("refs/heads/*").unwrap().count()
					== 1
			);
		}
