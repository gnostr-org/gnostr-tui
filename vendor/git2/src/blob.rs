...
impl<'repo> Blob<'repo> {
	...
	pub fn content(&self) -> &[u8] {
		unsafe {
			let data =
				raw::git_blob_rawcontent(&*self.raw) as *const u8;
...
impl<'repo> BlobWriter<'repo> {
	...
	pub fn commit(mut self) -> Result<Oid, Error> {
		...
		unsafe {
			try_call!(raw::git_blob_create_fromstream_commit(
				&mut raw, self.raw
			));
	...
	fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
		unsafe {
			if let Some(f) = (*self.raw).write {
				let res =
					f(self.raw, buf.as_ptr() as *const _, buf.len());
				if res < 0 {
					Err(io::Error::new(
						io::ErrorKind::Other,
						"Write error",
...
mod tests {
	...
	fn stream() {
		...
		let repo = Repository::init(td.path()).unwrap();
		let mut ws =
			repo.blob_writer(Some(Path::new("foo"))).unwrap();
