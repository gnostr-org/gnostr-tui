...
pub struct Progress<'a> {
	...
	pub(crate) _marker:
		marker::PhantomData<&'a raw::git_indexer_progress>,
}
...
impl<'a> Binding for Progress<'a> {
	...
	unsafe fn from_raw(
		raw: *const raw::git_indexer_progress,
	) -> Progress<'a> {
		Progress {
...
impl<'a> Indexer<'a> {
	...
	pub fn commit(mut self) -> Result<String, Error> {
		unsafe {
			...
			let name =
				CStr::from_ptr(raw::git_indexer_name(self.raw));
...
impl io::Write for Indexer<'_> {
	fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
		unsafe {
			...
			let res = raw::git_indexer_append(
				self.raw,
				ptr,
				len,
				&mut self.progress,
