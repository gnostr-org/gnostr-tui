...
macro_rules! from_value {
	($value:expr => $string:expr) => {
		match unsafe {
			raw::git_attr_value(
				$value.map_or(ptr::null(), |v| v.as_ptr().cast()),
			)
		} {
...
impl PartialEq for AttrValue<'_> {
	fn eq(&self, other: &AttrValue<'_>) -> bool {
		match (self, other) {
			...
			| (AttrValue::Bytes(bytes), AttrValue::String(string)) => {
				string.as_bytes() == *bytes
			}
			(AttrValue::String(left), AttrValue::String(right)) => {
				left == right
			}
			(AttrValue::Bytes(left), AttrValue::Bytes(right)) => {
				left == right
			}
			_ => false,
		}
...
mod tests {
	...
	fn attr_value_partial_eq() {
		...
		assert_eq!(
			AttrValue::Bytes(b"foo"),
			AttrValue::Bytes(b"foo")
		);
		assert_eq!(
			AttrValue::String("bar"),
			AttrValue::Bytes(b"bar")
		);
		assert_eq!(
			AttrValue::Bytes(b"bar"),
			AttrValue::String("bar")
		...
		assert_ne!(
			...
			AttrValue::Bytes(b"unspecified")
		);
		assert_ne!(AttrValue::Bytes(b"false"), AttrValue::False);
		assert_ne!(
			AttrValue::String("unspecified"),
			AttrValue::Unspecified
		);
		assert_ne!(
			AttrValue::String("foo"),
