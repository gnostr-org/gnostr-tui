...
impl Error {
	...
	pub fn last_error(code: c_int) -> Option<Error> {
		...
		unsafe {
			...
			let err = if ptr.is_null() {
				let mut error =
					Error::from_str("an unknown git error occurred");
				error.code = code;
				error
			} else {
	...
	unsafe fn from_raw(
		code: c_int,
		ptr: *const raw::git_error,
	) -> Error {
		let message =
			CStr::from_ptr((*ptr).message as *const _).to_bytes();
	...
	pub fn code(&self) -> ErrorCode {
		match self.raw_code() {
			...
			raw::GIT_ENONFASTFORWARD => {
				super::ErrorCode::NotFastForward
			}
			...
			raw::GIT_EMERGECONFLICT => {
				super::ErrorCode::MergeConflict
			}
	...
	pub fn class(&self) -> ErrorClass {
		match self.raw_class() {
			...
			raw::GIT_ERROR_REPOSITORY => {
				super::ErrorClass::Repository
			}
			...
			raw::GIT_ERROR_CHERRYPICK => {
				super::ErrorClass::CherryPick
			}
			raw::GIT_ERROR_DESCRIBE => super::ErrorClass::Describe,
			raw::GIT_ERROR_REBASE => super::ErrorClass::Rebase,
			raw::GIT_ERROR_FILESYSTEM => {
				super::ErrorClass::Filesystem
			}
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		...
		match self.class() {
			ErrorClass::None => {}
			other => {
				write!(f, "; class={:?} ({})", other, self.klass)?
			}
		}
		match self.code() {
...
mod tests {
	...
	fn smoke() {
		...
		let err =
			repo.find_submodule("does_not_exist").err().unwrap();
