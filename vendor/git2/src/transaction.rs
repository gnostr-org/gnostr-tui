...
use crate::{
	raw, util::Binding, Error, Oid, Reflog, Repository, Signature,
};
...
impl<'repo> Transaction<'repo> {
	...
	pub fn lock_ref(&mut self, refname: &str) -> Result<(), Error> {
		...
		unsafe {
			try_call!(raw::git_transaction_lock_ref(
				self.raw, refname
			));
		}
...
mod tests {
	...
	fn locks_same_repo_handle() {
		...
		assert!(
			matches!(tx2.lock_ref("refs/heads/seen"), Err(e) if e.code() == ErrorCode::Locked)
		)
	}
	...
	fn remove() {
		...
		assert!(
			matches!(repo.refname_to_id(next), Err(e) if e.code() == ErrorCode::NotFound)
		)
	}
