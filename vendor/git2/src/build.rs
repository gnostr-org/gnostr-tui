...
use crate::{
	panic, raw, Error, FetchOptions, IntoCString, Oid, Repository,
	Tree,
};
...
pub type RemoteCreate<'cb> = dyn for<'a> FnMut(
		&'a Repository,
		&str,
		&str,
...
impl<'cb> RepoBuilder<'cb> {
	...
	pub fn clone(
		...
		let mut opts: raw::git_clone_options =
			unsafe { mem::zeroed() };
		unsafe {
...
impl<'cb> CheckoutBuilder<'cb> {
	...
	fn flag(
		&mut self,
		bit: raw::git_checkout_strategy_t,
		on: bool,
	...
	pub fn overwrite_ignored(
		...
		self.flag(
			raw::GIT_CHECKOUT_DONT_OVERWRITE_IGNORED,
			!overwrite,
		)
	}
...
extern "C" fn notify_cb(
	...
	panic::wrap(|| unsafe {
		...
		let why =
			CheckoutNotificationType::from_bits_truncate(why as u32);
		let keep_going =
			callback(why, path, baseline, target, workdir);
		if keep_going {
			0
		} else {
			1
...
mod tests {
	...
	fn smoke() {
		let r = RepoBuilder::new()
			.clone("/path/to/nowhere", Path::new("foo"));
		assert!(r.is_err());
	}
	...
	fn smoke_tree_create_updated() {
		...
		let baz_id =
			tree2.get_path(Path::new("bar/baz")).unwrap().id();
	...
	fn notify_callback() {
		...
		let repo =
			Repository::open_bare(&td.path().join(".git")).unwrap();
		let tree = repo
		...
		checkout_opts.notify(
			|_notif, _path, baseline, target, workdir| {
