...
use crate::{
	panic, raw, Cred, CredentialType, Error, IndexerProgress, Oid,
	PackBuilderStage, Progress, PushUpdate,
};
...
pub type Credentials<'a> = dyn FnMut(&str, Option<&str>, CredentialType) -> Result<Cred, Error>
	+ 'a;
...
/// argument.
pub type CertificateCheck<'a> = dyn FnMut(&Cert<'_>, &str) -> Result<CertificateCheckStatus, Error>
	+ 'a;
...
pub type PushUpdateReference<'a> =
	dyn FnMut(&str, Option<&str>) -> Result<(), Error> + 'a;
...
///     * total
///     * bytes
pub type PushTransferProgress<'a> =
	dyn FnMut(usize, usize, usize) + 'a;
...
///     * current
///     * total
pub type PackProgress<'a> =
	dyn FnMut(PackBuilderStage, usize, usize) + 'a;
...
pub type PushNegotiation<'a> =
	dyn FnMut(&[PushUpdate<'_>]) -> Result<(), Error> + 'a;
...
impl<'a> RemoteCallbacks<'a> {
	...
	{
		self.progress =
			Some(Box::new(cb) as Box<IndexerProgress<'a>>);
		self
	}
			...
			+ 'a,
	{
		self.certificate_check =
			Some(Box::new(cb) as Box<CertificateCheck<'a>>);
		self
	}
	...
	pub fn push_update_reference<F>(
		&mut self,
		cb: F,
	) -> &mut RemoteCallbacks<'a>
	where
		F: FnMut(&str, Option<&str>) -> Result<(), Error> + 'a,
	{
		self.push_update_reference =
			Some(Box::new(cb) as Box<PushUpdateReference<'a>>);
		self
	}

	/// The callback through which progress of push transfer is monitored
	pub fn push_transfer_progress<F>(
		&mut self,
		cb: F,
	) -> &mut RemoteCallbacks<'a>
	where
	...
	{
		self.push_progress =
			Some(Box::new(cb) as Box<PushTransferProgress<'a>>);
		self
	}
	...
	{
		self.pack_progress =
			Some(Box::new(cb) as Box<PackProgress<'a>>);
		self
	}
	...
	{
		self.push_negotiation =
			Some(Box::new(cb) as Box<PushNegotiation<'a>>);
		self
	}
}
...
impl<'a> Binding for RemoteCallbacks<'a> {
	...
	fn raw(&self) -> raw::git_remote_callbacks {
		unsafe {
			let mut callbacks: raw::git_remote_callbacks =
				mem::zeroed();
			assert_eq!(
				raw::git_remote_init_callbacks(
					&mut callbacks,
	...
	unsafe {
		let ok = panic::wrap(|| {
			...
			*ret = ptr::null_mut();
			...
			let username_from_url =
				match crate::opt_bytes(&url, username_from_url) {
					Some(username) => {
						Some(str::from_utf8(username).map_err(
							|_| raw::GIT_PASSTHROUGH as c_int,
						)?)
					}
					None => None,
				};

			let cred_type = CredentialType::from_bits_truncate(
				allowed_types as u32,
			);

			callback(url, username_from_url, cred_type).map_err(|e| {
				let s = CString::new(e.to_string()).unwrap();
				raw::git_error_set_str(
					e.class() as c_int,
					s.as_ptr(),
				);
				e.raw_code() as c_int
...
extern "C" fn sideband_progress_cb(
	str: *const c_char,
	len: c_int,
	payload: *mut c_void,
) -> c_int {
...
extern "C" fn update_tips_cb(
	...
	let ok = panic::wrap(|| unsafe {
		...
		let refname =
			str::from_utf8(CStr::from_ptr(refname).to_bytes())
				.unwrap();
...
extern "C" fn certificate_check_cb(
	...
	let ok = panic::wrap(|| unsafe {
		let payload = &mut *(data as *mut RemoteCallbacks<'_>);
		let callback =
			match payload.certificate_check {
				Some(ref mut c) => c,
				None => return Ok(
		...
		let hostname =
			str::from_utf8(CStr::from_ptr(hostname).to_bytes())
				.unwrap();
...
extern "C" fn push_update_reference_cb(
	...
	panic::wrap(|| unsafe {
		...
		let status = if status.is_null() {
			...
			Some(
				str::from_utf8(CStr::from_ptr(status).to_bytes())
					.unwrap(),
			)
		};
...
extern "C" fn push_negotiation_cb(
	...
	panic::wrap(|| unsafe {
		...
		let updates = slice::from_raw_parts(
			updates as *mut PushUpdate<'_>,
			len,
		);
