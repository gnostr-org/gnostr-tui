...
impl Reflog {
	...
	pub fn get(&self, i: usize) -> Option<ReflogEntry<'_>> {
		unsafe {
			let ptr =
				raw::git_reflog_entry_byindex(self.raw, i as size_t);
...
impl<'reflog> ReflogEntry<'reflog> {
	...
	pub fn id_new(&self) -> Oid {
		unsafe {
			Binding::from_raw(raw::git_reflog_entry_id_new(self.raw))
		}
	}
	...
	pub fn id_old(&self) -> Oid {
		unsafe {
			Binding::from_raw(raw::git_reflog_entry_id_old(self.raw))
		}
	}
	...
	pub fn message_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_reflog_entry_message(self.raw),
			)
		}
	}
}
