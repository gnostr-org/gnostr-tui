...
impl<'a> CertHostkey<'a> {
	...
	pub fn hash_md5(&self) -> Option<&[u8; 16]> {
		unsafe {
			if (*self.raw).kind as u32 & raw::GIT_CERT_SSH_MD5 as u32
				== 0
			{
				None
			} else {
	...
	pub fn hash_sha1(&self) -> Option<&[u8; 20]> {
		unsafe {
			if (*self.raw).kind as u32 & raw::GIT_CERT_SSH_SHA1 as u32
				== 0
			{
				None
			} else {
	...
	pub fn hash_sha256(&self) -> Option<&[u8; 32]> {
		unsafe {
			if (*self.raw).kind as u32
				& raw::GIT_CERT_SSH_SHA256 as u32
				== 0
			{
				None
			} else {
	...
	pub fn hostkey_type(&self) -> Option<SshHostKeyType> {
		unsafe {
			...
			let t = match (*self.raw).raw_type {
				raw::GIT_CERT_SSH_RAW_TYPE_UNKNOWN => {
					SshHostKeyType::Unknown
				}
				raw::GIT_CERT_SSH_RAW_TYPE_RSA => SshHostKeyType::Rsa,
				raw::GIT_CERT_SSH_RAW_TYPE_DSS => SshHostKeyType::Dss,
				raw::GIT_CERT_SSH_RAW_TYPE_KEY_ECDSA_256 => {
					SshHostKeyType::Ecdsa256
				}
				raw::GIT_CERT_SSH_RAW_TYPE_KEY_ECDSA_384 => {
					SshHostKeyType::Ecdsa384
				}
				raw::GIT_CERT_SSH_RAW_TYPE_KEY_ECDSA_521 => {
					SshHostKeyType::Ecdsa521
				}
				raw::GIT_CERT_SSH_RAW_TYPE_KEY_ED25519 => {
