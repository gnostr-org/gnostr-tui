...
use crate::diff::{
	binary_cb_c, file_cb_c, hunk_cb_c, line_cb_c, BinaryCb,
	DiffCallbacks, FileCb, HunkCb, LineCb,
};
use crate::oid_array::OidArray;
use crate::stash::{
	stash_cb, StashApplyOptions, StashCbData, StashSaveOptions,
};
use crate::string_array::StringArray;
use crate::tagforeach::{
	tag_foreach_cb, TagForeachCB, TagForeachData,
};
...
use crate::{
	raw, AttrCheckFlags, Buf, Error, Object, Remote,
	RepositoryOpenFlags, RepositoryState, Revspec, StashFlags,
};
use crate::{
	AnnotatedCommit, MergeAnalysis, MergeOptions, MergePreference,
	SubmoduleIgnore, SubmoduleStatus, SubmoduleUpdate,
};
use crate::{ApplyLocation, ApplyOptions, Rebase, RebaseOptions};
use crate::{
	Blame, BlameOptions, Reference, References, ResetType, Signature,
	Submodule,
};
use crate::{
	Blob, BlobWriter, Branch, BranchType, Branches, Commit, Config,
	Index, Oid, Tree,
};
use crate::{
	Describe, IntoCString, Reflog, RepositoryInitMode, RevparseMode,
};
use crate::{
	DescribeOptions, Diff, DiffOptions, Odb, PackBuilder, TreeBuilder,
};
use crate::{
	Note, Notes, ObjectType, Revwalk, Status, StatusOptions,
	Statuses, Tag, Transaction,
};

type MergeheadForeachCb<'a> = dyn FnMut(&Oid) -> bool + 'a;
type FetchheadForeachCb<'a> =
	dyn FnMut(&str, &[u8], &Oid, bool) -> bool + 'a;
...
extern "C" fn mergehead_foreach_cb(
...
extern "C" fn fetchhead_foreach_cb(
	...
	panic::wrap(|| unsafe {
		...
		let res = {
			...
			let ref_name =
				str::from_utf8(CStr::from_ptr(ref_name).to_bytes())
					.unwrap();
...
impl Repository {
	...
	pub fn discover_path<P: AsRef<Path>, I, O>(
		path: P,
		ceiling_dirs: I,
	...
	pub fn init_bare<P: AsRef<Path>>(
		path: P,
	) -> Result<Repository, Error> {
		Repository::init_opts(
			path,
			RepositoryInitOptions::new().bare(true),
		)
	}
	...
	pub fn init_opts<P: AsRef<Path>>(
		...
		unsafe {
			...
			try_call!(raw::git_repository_init_ext(
				&mut ret, path, &mut opts
			));
			Ok(Binding::from_raw(ret))
		}
	}
	...
	pub fn from_odb(odb: Odb<'_>) -> Result<Repository, Error> {
		...
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_repository_wrap_odb(
				&mut ret,
				odb.raw()
			));
			Ok(Binding::from_raw(ret))
	...
	pub fn revparse(&self, spec: &str) -> Result<Revspec<'_>, Error> {
		...
		unsafe {
			...
			let mode =
				RevparseMode::from_bits_truncate(raw.flags as u32);
	...
	pub fn revparse_single(
		...
		unsafe {
			try_call!(raw::git_revparse_single(
				&mut obj, self.raw, spec
			));
			assert!(!obj.is_null());
	...
	pub fn revparse_ext(
		...
		unsafe {
			...
			Ok((
				Binding::from_raw(git_obj),
				Binding::from_raw_opt(git_ref),
			))
		}
	}
	...
	pub fn namespace_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_repository_get_namespace(self.raw),
			)
		}
	}
	...
	pub fn set_namespace_bytes(
		&self,
		namespace: &[u8],
	) -> Result<(), Error> {
		unsafe {
			let namespace = CString::new(namespace)?;
			try_call!(raw::git_repository_set_namespace(
				self.raw, namespace
			));
			Ok(())
		}
	...
	pub fn message(&self) -> Result<String, Error> {
		unsafe {
			let buf = Buf::new();
			try_call!(raw::git_repository_message(
				buf.raw(),
				self.raw
			));
	...
	pub fn find_remote(
		...
		unsafe {
			try_call!(raw::git_remote_lookup(
				&mut ret, self.raw, name
			));
			Ok(Binding::from_raw(ret))
		}
	}
	...
	pub fn remote(
		...
		unsafe {
			try_call!(raw::git_remote_create(
				&mut ret, self.raw, name, url
			));
			Ok(Binding::from_raw(ret))
		}
	}
	...
	pub fn remote_anonymous(
		...
		let url = CString::new(url)?;
		unsafe {
			try_call!(raw::git_remote_create_anonymous(
				&mut ret, self.raw, url
			));
			Ok(Binding::from_raw(ret))
		}
	}
	...
	pub fn remote_add_fetch(
		...
		let spec = CString::new(spec)?;
		unsafe {
			try_call!(raw::git_remote_add_fetch(
				self.raw, name, spec
			));
		}
		Ok(())
	...
	pub fn remote_set_pushurl(
		...
		unsafe {
			try_call!(raw::git_remote_set_pushurl(
				self.raw, name, pushurl
			));
		}
		Ok(())
	}
		...
		I: IntoIterator<Item = T>,
	{
		...
		unsafe {
			try_call!(raw::git_reset_default(
				self.raw, target, &mut arr
			));
		}
		Ok(())
	}
	...
	/// Otherwise, the HEAD will be detached and will directly point to the
	/// commit.
	pub fn set_head_bytes(
		&self,
		refname: &[u8],
	) -> Result<(), Error> {
		let refname = CString::new(refname)?;
		unsafe {
			try_call!(raw::git_repository_set_head(
				self.raw, refname
			));
		}
		Ok(())
	...
	pub fn set_head_detached_from_annotated(
		...
		unsafe {
			try_call!(
				raw::git_repository_set_head_detached_from_annotated(
					self.raw,
					commitish.raw()
				)
			);
	...
	pub fn references(&self) -> Result<References<'_>, Error> {
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_reference_iterator_new(
				&mut ret, self.raw
			));
			Ok(Binding::from_raw(ret))
		}
	...
	pub fn submodules(&self) -> Result<Vec<Submodule<'_>>, Error> {
		...
		extern "C" fn append(
			...
			unsafe {
				...
				let rc = raw::git_submodule_lookup(
					&mut raw,
					data.repo.raw(),
					name,
				);
	...
	pub fn branches(
		...
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_branch_iterator_new(
				&mut raw,
				self.raw(),
				filter
			));
	...
	pub fn index(&self) -> Result<Index, Error> {
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_repository_index(
				&mut raw,
				self.raw()
			));
	...
	pub fn set_index(&self, index: &mut Index) -> Result<(), Error> {
		unsafe {
			try_call!(raw::git_repository_set_index(
				self.raw(),
				index.raw()
			));
		}
	...
	pub fn config(&self) -> Result<Config, Error> {
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_repository_config(
				&mut raw,
				self.raw()
			));
			Ok(Binding::from_raw(raw))
	...
	pub fn blob_path(&self, path: &Path) -> Result<Oid, Error> {
		...
		unsafe {
			try_call!(raw::git_blob_create_fromdisk(
				&mut raw,
				self.raw(),
				path
			));
	...
	pub fn set_odb(&self, odb: &Odb<'_>) -> Result<(), Error> {
		unsafe {
			try_call!(raw::git_repository_set_odb(
				self.raw(),
				odb.raw()
			));
		}
	...
	pub fn find_commit(&self, oid: Oid) -> Result<Commit<'_>, Error> {
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_commit_lookup(
				&mut raw,
				self.raw(),
				oid.raw()
			));
	...
	pub fn find_reference(
		...
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_reference_lookup(
				&mut raw,
				self.raw(),
				name
			));
	...
	pub fn resolve_reference_from_short_name(
		...
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_reference_dwim(
				&mut raw,
				self.raw(),
				refname
			));
	...
	pub fn refname_to_id(&self, name: &str) -> Result<Oid, Error> {
		...
		unsafe {
			try_call!(raw::git_reference_name_to_id(
				&mut ret,
				self.raw(),
				name
			));
	...
	pub fn signature(&self) -> Result<Signature<'static>, Error> {
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_signature_default(
				&mut ret,
				self.raw()
			));
			Ok(Binding::from_raw(ret))
	...
	pub fn find_submodule(
		...
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_submodule_lookup(
				&mut raw,
				self.raw(),
				name
			));
	...
	pub fn submodule_status(
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_submodule_status(
				&mut ret, self.raw, name, ignore
			));
		}
	...
	pub fn submodule_set_ignore(
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_submodule_set_ignore(
				self.raw(),
				name,
				ignore
			));
	...
	pub fn submodule_set_update(
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_submodule_set_update(
				self.raw(),
				name,
				update
			));
	...
	pub fn submodule_set_url(
		...
		let url = CString::new(url)?;
		unsafe {
			try_call!(raw::git_submodule_set_url(
				self.raw(),
				name,
				url
			));
	...
	pub fn submodule_set_branch(
		...
		let branch_name = CString::new(branch_name)?;
		unsafe {
			try_call!(raw::git_submodule_set_branch(
				self.raw(),
				name,
				branch_name
			));
	...
	pub fn treebuilder(
		...
		unsafe {
			...
			try_call!(raw::git_treebuilder_new(
				&mut ret, self.raw, tree
			));
			Ok(Binding::from_raw(ret))
		}
	}
	...
	pub fn tag_names(
		...
		unsafe {
			match pattern {
				Some(s) => {
					let s = CString::new(s)?;
					try_call!(raw::git_tag_list_match(
						&mut arr, s, self.raw
					));
				}
				None => {
	...
	pub fn checkout_tree(
		...
		unsafe {
			...
			try_call!(raw::git_checkout_tree(
				self.raw,
				&*treeish.raw(),
				&raw_opts
			));
	...
	pub fn merge_analysis(
		...
		unsafe {
			let mut raw_merge_analysis =
				0 as raw::git_merge_analysis_t;
			let mut raw_merge_preference =
				0 as raw::git_merge_preference_t;
			...
			Ok((
				MergeAnalysis::from_bits_truncate(
					raw_merge_analysis as u32,
				),
				MergePreference::from_bits_truncate(
					raw_merge_preference as u32,
				),
			))
		}
	}
	...
	pub fn notes(
		&self,
		notes_ref: Option<&str>,
	) -> Result<Notes<'_>, Error> {
		let notes_ref = crate::opt_cstr(notes_ref)?;
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_note_iterator_new(
				&mut ret, self.raw, notes_ref
			));
			Ok(Binding::from_raw(ret))
		}
	}
	...
	pub fn reflog_rename(
		...
		let new_name = CString::new(new_name)?;
		unsafe {
			try_call!(raw::git_reflog_rename(
				self.raw, old_name, new_name
			));
		}
		Ok(())
	}
	...
	) -> Result<Describe<'_>, Error> {
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_describe_workdir(
				&mut ret,
				self.raw,
				opts.raw()
			));
	...
	pub fn diff_blobs(
		...
		unsafe {
			...
			let binary_cb_c: raw::git_diff_binary_cb =
				if cbs.binary.is_some() {
					Some(binary_cb_c)
				} else {
					None
				};
			let hunk_cb_c: raw::git_diff_hunk_cb =
				if cbs.hunk.is_some() {
					Some(hunk_cb_c)
				} else {
					None
				};
			let line_cb_c: raw::git_diff_line_cb =
				if cbs.line.is_some() {
					Some(line_cb_c)
				} else {
					None
	...
	pub fn is_path_ignored<P: AsRef<Path>>(
		&self,
		path: P,
	) -> Result<bool, Error> {
	...
	pub fn branch_remote_name(
		...
		unsafe {
			let buf = Buf::new();
			try_call!(raw::git_branch_remote_name(
				buf.raw(),
				self.raw,
				refname
			));
	...
	pub fn branch_upstream_name(
		...
		unsafe {
			let buf = Buf::new();
			try_call!(raw::git_branch_upstream_name(
				buf.raw(),
				self.raw,
				refname
			));
	...
	pub fn find_worktree(
		...
		unsafe {
			try_call!(raw::git_worktree_lookup(
	...
	pub fn mailmap(&self) -> Result<Mailmap, Error> {
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_mailmap_from_repository(
				&mut ret, self.raw
			));
			Ok(Binding::from_raw(ret))
		}
		...
		C: FnMut(&Oid) -> bool,
	{
		unsafe {
			...
			let cb: raw::git_repository_mergehead_foreach_cb =
				Some(mergehead_foreach_cb);
		...
		C: FnMut(&str, &[u8], &Oid, bool) -> bool,
	{
		unsafe {
			...
			let cb: raw::git_repository_fetchhead_foreach_cb =
				Some(fetchhead_foreach_cb);
...
impl RepositoryInitOptions {
	...
	pub unsafe fn raw(&self) -> raw::git_repository_init_options {
		...
		opts.template_path =
			crate::call::convert(&self.template_path);
...
mod tests {
	...
	use crate::CherrypickOptions;
	use crate::{
		ObjectType, Oid, Repository, ResetType, Signature,
		SubmoduleIgnore, SubmoduleUpdate,
	};
	use std::ffi::OsStr;
	use std::fs;
	use std::path::Path;
	...
	fn smoke_revparse() {
		...
		assert_eq!(
			repo.revparse_single("HEAD").unwrap().id(),
			from.id()
		);
	...
	fn smoke_discover_path() {
		...
		Repository::init_bare(td.path()).unwrap();
		let path =
			Repository::discover_path(&subdir, &[] as &[&OsStr])
				.unwrap();
		assert_eq!(
	...
	fn smoke_discover_path_ceiling_dir() {
		...
		Repository::init_bare(td.path()).unwrap();
		let path = Repository::discover_path(
			&testdir,
			&[ceilingdir.as_os_str()],
		);
	...
	fn smoke_open_ext() {
		...
		let repo = Repository::open_ext(
			&subdir,
			crate::RepositoryOpenFlags::BARE,
			&[] as &[&OsStr],
		)
		.unwrap();
		assert!(repo.is_bare());
	...
	fn smoke_graph_ahead_behind() {
		...
		let (ahead, behind) =
			repo.graph_ahead_behind(head_id, head_parent_id).unwrap();
		assert_eq!(ahead, 1);
		assert_eq!(behind, 0);
		let (ahead, behind) =
			repo.graph_ahead_behind(head_parent_id, head_id).unwrap();
		assert_eq!(ahead, 0);
		assert_eq!(behind, 1);
	}
	...
	fn smoke_graph_descendant_of() {
		...
		let head_parent_id = head.parent(0).unwrap().id();
		assert!(repo
			.graph_descendant_of(head_id, head_parent_id)
			.unwrap());
		assert!(!repo
			.graph_descendant_of(head_parent_id, head_id)
			.unwrap());
	}
	...
	fn smoke_reference_has_log_ensure_log() {
		...
		assert_eq!(
			repo.reference_has_log("refs/heads/main").unwrap(),
			true
		);
		assert_eq!(
			repo.reference_has_log("NOT_HEAD").unwrap(),
			false
		);
	...
	fn smoke_set_head_bytes() {
		...
		assert!(repo
			.set_head_bytes(b"refs/heads/does-not-exist")
			.is_ok());
		assert!(repo.head().is_err());
	...
	fn smoke_set_head_detached() {
		...
		let void_oid =
			Oid::from_bytes(b"00000000000000000000").unwrap();
	...
	fn smoke_merge_base() {
		...
		let merge_base =
			repo.merge_base_many(&[oid2, oid3, oid4]).unwrap();
		assert_eq!(merge_base, oid1);
	}
	...
	fn smoke_merge_bases() {
		...
		// merge bases of (oid4,oid5) should be (oid2,oid3)
		let merge_bases =
			repo.merge_bases_many(&[oid4, oid5]).unwrap();
		let mut found_oid2 = false;
		let mut found_oid3 = false;
		for mg in merge_bases.iter() {
			println!("found merge base {:?}", mg);
	...
	fn smoke_revparse_ext() {
...

		{
			...
			let (obj, reference) =
				repo.revparse_ext(short_refname).unwrap();
			let expected_obj =
				repo.revparse_single(expected_refname).unwrap();
			assert_eq!(obj.id(), expected_obj.id());
			assert_eq!(
				reference.unwrap().name().unwrap(),
				expected_refname
			);
		}
		{
	...
	fn smoke_is_path_ignored() {
		...
		if cfg!(windows) {
			assert!(repo
				.is_path_ignored(Path::new("foo\\thing"))
				.unwrap());
		}
	...
	fn smoke_config_write_and_read() {
		...
		let c =
			fs::read_to_string(td.path().join(".git").join("config"))
				.unwrap();
	...
	fn smoke_merge_analysis_for_ref() -> Result<(), crate::Error> {
		...
		let their_branch =
			repo.branch("their-branch", &head_commit, false)?;
		...
		let (merge_analysis, _merge_preference) = repo
			.merge_analysis_for_ref(
				main.get(),
				&[&repo.reference_to_annotated_commit(
					their_branch.get(),
				)?],
			)?;

		assert!(merge_analysis
			.contains(crate::MergeAnalysis::ANALYSIS_FASTFORWARD));
	...
	fn smoke_submodule_set() -> Result<(), crate::Error> {
		...
		{
			let mut s =
				repo2.submodule(&url, Path::new(name), true)?;
		...
		// ignore rule
		repo2
			.submodule_set_ignore(name, SubmoduleIgnore::Untracked)?;
		assert!(matches!(
	...
	fn smoke_mailmap_from_repository() {
		...
		let mailmapped_author =
			t!(commit.author_with_mailmap(&mailmap));
		let mailmapped_committer =
			t!(commit.committer_with_mailmap(&mailmap));
		...
		let commit = {
			...
			let p = Path::new(repo.workdir().unwrap())
				.join(&mailmap_file);
			t!(fs::write(
				p,
				r#"
...
Committer Name <committer.proper@email> <committer@email>"#,
		...
		let mm_resolve_author =
			t!(mailmap.resolve_signature(&author));
		let mm_resolve_committer =
			t!(mailmap.resolve_signature(&committer));
		...
		assert_eq!(
			mailmapped_author.email(),
			Some("author.proper@email")
		);
		assert_eq!(
			mailmapped_committer.name(),
			Some("Committer Name")
		);
		assert_eq!(
			mailmapped_committer.email(),
			Some("committer.proper@email")
		);

		// resolve_signature() works
		assert_eq!(
			mm_resolve_author.email(),
			mailmapped_author.email()
		);
		assert_eq!(
			mm_resolve_committer.email(),
			mailmapped_committer.email()
		);
	}
}
