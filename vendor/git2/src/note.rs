...
impl<'repo> Note<'repo> {
	...
	pub fn committer(&self) -> Signature<'_> {
		unsafe {
			signature::from_raw_const(
				self,
				raw::git_note_committer(&*self.raw),
			)
		}
	}
	...
	pub fn message_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_note_message(&*self.raw))
				.unwrap()
		}
	}
...
mod tests {
	...
	fn smoke() {
		...
		let note =
			repo.note(&sig, &sig, None, head, "foo", false).unwrap();
		...
		let (a, b) =
			repo.notes(None).unwrap().next().unwrap().unwrap();
		assert_eq!(a, note);
		assert_eq!(b, head);

		assert_eq!(
			repo.note_default_ref().unwrap(),
			"refs/notes/commits"
		);
