...
use crate::{
	call, raw, Buf, Direction, Error, FetchPrune, Oid, ProxyOptions,
	Refspec,
};
...
impl<'repo> Remote<'repo> {
	...
	pub fn create_detached<S: Into<Vec<u8>>>(
		url: S,
	...
	pub fn name_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(self, raw::git_remote_name(&*self.raw))
		}
	}
	...
	pub fn url_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_remote_url(&*self.raw))
				.unwrap()
		}
	}
	...
	pub fn pushurl_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_remote_pushurl(&*self.raw),
			)
		}
	}
	...
	pub fn default_branch(&self) -> Result<Buf, Error> {
		unsafe {
			let buf = Buf::new();
			try_call!(raw::git_remote_default_branch(
				buf.raw(),
				self.raw
			));
			Ok(buf)
	...
	pub fn connect_auth<'connection, 'cb>(
		...
		let proxy_options =
			proxy_options.unwrap_or_else(ProxyOptions::new);
		unsafe {
			try_call!(raw::git_remote_connect(
				self.raw,
				dir,
	...
	pub fn download<Str: AsRef<str> + crate::IntoCString + Clone>(
		...
		unsafe {
			try_call!(raw::git_remote_download(
				self.raw,
				&arr,
				raw.as_ref()
			));
	...
	pub fn refspecs(&self) -> Refspecs<'_> {
		let cnt = unsafe {
			raw::git_remote_refspec_count(&*self.raw) as usize
		};
		Refspecs {
			range: 0..cnt,
			remote: self,
	...
	pub fn get_refspec(&self, i: usize) -> Option<Refspec<'repo>> {
		unsafe {
			let ptr = raw::git_remote_get_refspec(
				&*self.raw,
				i as libc::size_t,
			);
	...
	pub fn list(&self) -> Result<&[RemoteHead<'_>], Error> {
		...
		unsafe {
			try_call!(raw::git_remote_ls(
				&mut base, &mut size, self.raw
			));
			assert_eq!(
	...
	pub fn prune(
		...
		let cbs =
			Box::new(callbacks.unwrap_or_else(RemoteCallbacks::new));
		unsafe {
	...
	pub fn fetch_refspecs(&self) -> Result<StringArray, Error> {
		unsafe {
			...
			try_call!(raw::git_remote_get_fetch_refspecs(
				&mut raw, self.raw
			));
	...
	pub fn push_refspecs(&self) -> Result<StringArray, Error> {
		unsafe {
			let mut raw: raw::git_strarray = mem::zeroed();
			try_call!(raw::git_remote_get_push_refspecs(
				&mut raw, self.raw
			));
			Ok(StringArray::from_raw(raw))
		}
...
impl<'repo> Clone for Remote<'repo> {
	fn clone(&self) -> Remote<'repo> {
		let mut ret = ptr::null_mut();
		let rc =
			unsafe { call!(raw::git_remote_dup(&mut ret, self.raw)) };
		assert_eq!(rc, 0);
		Remote {
			raw: ret,
...
impl<'remote> RemoteHead<'remote> {
	...
	pub fn name(&self) -> &str {
		let b = unsafe {
			crate::opt_bytes(self, (*self.raw).name).unwrap()
		};
	...
	pub fn symref_target(&self) -> Option<&str> {
		let b = unsafe {
			crate::opt_bytes(self, (*self.raw).symref_target)
		};
...
impl<'cb> FetchOptions<'cb> {
	...
	pub fn custom_headers(
		...
		self.custom_headers = custom_headers
			...
			.collect();
		self.custom_headers_ptrs =
			self.custom_headers.iter().map(|s| s.as_ptr()).collect();
		self
	}
}
...
impl<'cb> Binding for FetchOptions<'cb> {
	...
	fn raw(&self) -> raw::git_fetch_options {
		raw::git_fetch_options {
			...
			update_fetchhead: crate::call::convert(
				&self.update_fetchhead,
			),
...
impl<'cb> PushOptions<'cb> {
	...
	pub fn packbuilder_parallelism(
		&mut self,
		parallel: u32,
	) -> &mut Self {
...
impl<'repo, 'connection, 'cb>
	RemoteConnection<'repo, 'connection, 'cb>
{
...
impl<'repo, 'connection, 'cb> Drop
	for RemoteConnection<'repo, 'connection, 'cb>
{
	fn drop(&mut self) {
...
impl RemoteRedirect {
	fn raw(&self) -> raw::git_remote_redirect_t {
		match self {
			...
			RemoteRedirect::Initial => {
				raw::GIT_REMOTE_REDIRECT_INITIAL
			}
...
mod tests {
	...
	use crate::{
		Direction, FetchOptions, Remote, RemoteCallbacks, Repository,
	};
	use std::cell::Cell;
	...
	fn smoke() {
		...
		t!(repo.remote_set_pushurl(
			"origin",
			Some("/path/to/elsewhere")
		));
	...
	fn create_remote() {
...

		{
			let mut connection = origin
				.connect_auth(Direction::Push, None, None)
				.unwrap();
	...
	fn create_remote_anonymous() {
		...
		let origin =
			repo.remote_anonymous("/path/to/nowhere").unwrap();
		...
		fn assert_branch_count(repo: &Repository, count: usize) {
			assert_eq!(
				repo.branches(Some(crate::BranchType::Remote))
					.unwrap()
					.filter(|b| b.as_ref().unwrap().0.name().unwrap()
						== Some("origin/stale"))
					.count(),
				count,
			);
	...
	fn push_negotiation() {
		...
		let mut updated = false;
		{
			...
			callbacks.push_negotiation(|updates| {
				...
				let u = &updates[0];
				assert_eq!(
					u.src_refname().unwrap(),
					"refs/heads/main"
				);
				assert!(u.src().is_zero());
				assert_eq!(
					u.dst_refname().unwrap(),
					"refs/heads/main"
				);
				assert_eq!(u.dst(), oid);
			...
			callbacks.push_negotiation(|updates| {
				...
				for u in updates {
					...
					match src_name {
						...
						_ => {
							panic!("unexpected refname: {}", src_name)
						}
					}
				}
				Ok(())
