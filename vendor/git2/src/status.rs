...
impl StatusOptions {
	...
	pub fn new() -> StatusOptions {
		unsafe {
			...
			let r = raw::git_status_init_options(
				&mut raw,
	...
	pub fn show(&mut self, show: StatusShow) -> &mut StatusOptions {
		self.raw.show = match show {
			...
			StatusShow::IndexAndWorkdir => {
				raw::GIT_STATUS_SHOW_INDEX_AND_WORKDIR
			}
		};
		self
	}
	...
	/// working directory.
	pub fn renames_index_to_workdir(
		&mut self,
		include: bool,
	) -> &mut StatusOptions {
		self.flag(
			raw::GIT_STATUS_OPT_RENAMES_INDEX_TO_WORKDIR,
			include,
		)
	}
	...
	pub fn sort_case_insensitively(
		&mut self,
		include: bool,
	) -> &mut StatusOptions {
		self.flag(
			raw::GIT_STATUS_OPT_SORT_CASE_INSENSITIVELY,
			include,
		)
	}
	...
	pub fn include_unreadable_as_untracked(
		...
		self.flag(
			raw::GIT_STATUS_OPT_INCLUDE_UNREADABLE_AS_UNTRACKED,
			include,
		)
	}
...
impl<'repo> Statuses<'repo> {
	...
	pub fn get(&self, index: usize) -> Option<StatusEntry<'_>> {
		unsafe {
			let p =
				raw::git_status_byindex(self.raw, index as size_t);
...
impl<'statuses> StatusEntry<'statuses> {
	...
	pub fn path_bytes(&self) -> &[u8] {
		unsafe {
			if (*self.raw).head_to_index.is_null() {
				crate::opt_bytes(
					self,
					(*(*self.raw).index_to_workdir).old_file.path,
				)
			} else {
				crate::opt_bytes(
					self,
					(*(*self.raw).head_to_index).old_file.path,
				)
			}
			.unwrap()
		}
	...
	pub fn status(&self) -> Status {
		Status::from_bits_truncate(unsafe {
			(*self.raw).status as u32
		})
	}
...
mod tests {
	...
	fn gitignore() {
		let (td, repo) = crate::test::repo_init();
		t!(t!(File::create(td.path().join(".gitignore")))
			.write_all(b"foo\n"));
	...
	fn status_file() {
		...
		if cfg!(windows) {
			assert!(repo
				.status_file(Path::new("bar\\foo.txt"))
				.is_err());
		}
		t!(File::create(td.path().join("foo")));
		if cfg!(windows) {
		...
		if cfg!(windows) {
			let status =
				t!(repo.status_file(Path::new("bar\\foo.txt")));
			assert!(status.contains(crate::Status::WT_NEW));
		}
	}
}
