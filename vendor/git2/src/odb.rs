...
use crate::{
	raw, Error, IndexerProgress, Mempack, Object, ObjectType,
...
impl<'repo> Odb<'repo> {
	...
	pub fn packwriter(&self) -> Result<OdbPackwriter<'_>, Error> {
		let mut out = ptr::null_mut();
		let progress_cb: raw::git_indexer_progress_cb =
			Some(write_pack_progress_cb);
	...
	pub fn exists_ext(
		&self,
		oid: Oid,
		flags: OdbLookupFlags,
	) -> bool {
		unsafe {
			raw::git_odb_exists_ext(
				self.raw,
				oid.raw(),
	...
	pub fn add_disk_alternate(
		&self,
		path: &str,
	) -> Result<(), Error> {
		unsafe {
			let path = CString::new(path)?;
			try_call!(raw::git_odb_add_disk_alternate(
				self.raw, path
			));
			Ok(())
		}
...
impl<'a> OdbObject<'a> {
	...
	pub fn kind(&self) -> ObjectType {
		unsafe {
			ObjectType::from_raw(raw::git_odb_object_type(self.raw))
				.unwrap()
		}
	}
	...
	pub fn data(&self) -> &[u8] {
		unsafe {
			let size = self.len();
			let ptr: *const u8 =
				raw::git_odb_object_data(self.raw) as *const u8;
...
impl<'repo> OdbWriter<'repo> {
	...
	pub fn finalize(&mut self) -> Result<Oid, Error> {
		...
		unsafe {
			try_call!(raw::git_odb_stream_finalize_write(
				&mut raw, self.raw
			));
...
impl<'repo> OdbPackwriter<'repo> {
	...
	{
		let progress_payload = unsafe {
			&mut *(self.progress_payload_ptr
				as *mut OdbPackwriterCb<'_>)
		};

		progress_payload.cb =
			Some(Box::new(cb) as Box<IndexerProgress<'repo>>);
		self
	}
}
	...
	fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
		unsafe {
			...
			let res = match writepack.append {
				Some(append) => {
					append(self.raw, ptr, len, &mut self.progress)
				}
				None => -1,
			};
...
mod tests {
	...
	fn packwriter() {
		...
		let mut buf = Buf::new();
		let (commit_source_id, _tree) =
			crate::test::commit(&repo_source);
		...
		let commit_target =
			repo_target.find_commit(commit_source_id).unwrap();
