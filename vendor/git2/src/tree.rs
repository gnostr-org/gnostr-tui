...
impl<'repo> Tree<'repo> {
	...
	pub fn get_id(&self, id: Oid) -> Option<TreeEntry<'_>> {
		unsafe {
			let ptr =
				raw::git_tree_entry_byid(&*self.raw(), &*id.raw());
			if ptr.is_null() {
				None
			} else {
	...
	pub fn get(&self, n: usize) -> Option<TreeEntry<'_>> {
		unsafe {
			let ptr = raw::git_tree_entry_byindex(
				&*self.raw(),
				n as libc::size_t,
			);
			if ptr.is_null() {
	...
	pub fn get_name_bytes(
		&self,
		filename: &[u8],
	) -> Option<TreeEntry<'_>> {
		let filename = CString::new(filename).unwrap();
		unsafe {
			let ptr = call!(raw::git_tree_entry_byname(
				&*self.raw(),
				filename
			));
			if ptr.is_null() {
	...
	pub fn get_path(
		&self,
		path: &Path,
	) -> Result<TreeEntry<'static>, Error> {
		let path = path_to_repo_path(path)?;
		let mut ret = ptr::null_mut();
		unsafe {
			try_call!(raw::git_tree_entry_bypath(
				&mut ret,
				&*self.raw(),
				path
			));
...
pub unsafe fn entry_from_raw_const<'tree>(
...
impl<'tree> TreeEntry<'tree> {
	...
	pub fn id(&self) -> Oid {
		unsafe {
			Binding::from_raw(raw::git_tree_entry_id(&*self.raw))
		}
	}
	...
	pub fn name_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_tree_entry_name(&*self.raw()),
			)
			.unwrap()
		}
	}
	...
	pub fn kind(&self) -> Option<ObjectType> {
		ObjectType::from_raw(unsafe {
			raw::git_tree_entry_type(&*self.raw)
		})
	}
...
impl<'a> Clone for TreeEntry<'a> {
	fn clone(&self) -> TreeEntry<'a> {
		...
		unsafe {
			assert_eq!(
				raw::git_tree_entry_dup(&mut ret, &*self.raw()),
				0
			);
...
impl<'a> Ord for TreeEntry<'a> {
	fn cmp(&self, other: &TreeEntry<'a>) -> Ordering {
		c_cmp_to_ordering(unsafe {
			raw::git_tree_entry_cmp(&*self.raw(), &*other.raw())
		})
	}
}
			...
			if self.entries.is_empty() {
				...
				match entry.kind() {
					Some(ObjectType::Tree) => {
						let obj: Object<'a> =
							entry.to_object(self.repo).unwrap();
