...
use crate::{
	raw, Diff, DiffDelta, Error, Index, IntoCString, PathspecFlags,
	Repository, Tree,
};
...
impl<'ps> PathspecMatchList<'ps> {
	fn entrycount(&self) -> usize {
		unsafe {
			raw::git_pathspec_match_list_entrycount(&*self.raw)
				as usize
		}
	}
	...
	fn failed_entrycount(&self) -> usize {
		unsafe {
			raw::git_pathspec_match_list_failed_entrycount(&*self.raw)
				as usize
		}
	}
	...
	pub fn entry(&self, i: usize) -> Option<&[u8]> {
		unsafe {
			let ptr = raw::git_pathspec_match_list_entry(
				&*self.raw,
				i as size_t,
			);
	...
	pub fn diff_entry(&self, i: usize) -> Option<DiffDelta<'_>> {
		unsafe {
			let ptr = raw::git_pathspec_match_list_diff_entry(
				&*self.raw,
				i as size_t,
			);
	...
	pub fn failed_entry(&self, i: usize) -> Option<&[u8]> {
		unsafe {
			let ptr = raw::git_pathspec_match_list_failed_entry(
				&*self.raw,
				i as size_t,
			);
			crate::opt_bytes(self, ptr)
...
mod tests {
	...
	fn smoke() {
		...
		assert!(
			ps.matches_path(Path::new("a"), PathspecFlags::DEFAULT)
		);
		assert!(
			ps.matches_path(Path::new("a/b"), PathspecFlags::DEFAULT)
		);
		assert!(
			!ps.matches_path(Path::new("b"), PathspecFlags::DEFAULT)
		);
		assert!(!ps
			.matches_path(Path::new("ab/c"), PathspecFlags::DEFAULT));

		let (td, repo) = crate::test::repo_init();
		let list =
			ps.match_workdir(&repo, PathspecFlags::DEFAULT).unwrap();
