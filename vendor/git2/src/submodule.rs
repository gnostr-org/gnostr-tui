...
use crate::{
	build::CheckoutBuilder, SubmoduleIgnore, SubmoduleUpdate,
};
...
impl<'repo> Submodule<'repo> {
	...
	pub fn branch_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_submodule_branch(self.raw),
			)
		}
	}
	...
	pub fn opt_url_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(self, raw::git_submodule_url(self.raw))
		}
	}
	...
	pub fn name_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(self, raw::git_submodule_name(self.raw))
				.unwrap()
		}
	}
	...
	pub fn path(&self) -> &Path {
		util::bytes2path(unsafe {
			crate::opt_bytes(self, raw::git_submodule_path(self.raw))
				.unwrap()
		})
	}
	...
	pub fn head_id(&self) -> Option<Oid> {
		unsafe {
			Binding::from_raw_opt(raw::git_submodule_head_id(
				self.raw,
			))
		}
	}
	...
	pub fn index_id(&self) -> Option<Oid> {
		unsafe {
			Binding::from_raw_opt(raw::git_submodule_index_id(
				self.raw,
			))
		}
	}
	...
	pub fn ignore_rule(&self) -> SubmoduleIgnore {
		SubmoduleIgnore::from_raw(unsafe {
			raw::git_submodule_ignore(self.raw)
		})
	}

	/// Get the update rule that will be used for the submodule.
	pub fn update_strategy(&self) -> SubmoduleUpdate {
		SubmoduleUpdate::from_raw(unsafe {
			raw::git_submodule_update_strategy(self.raw)
		})
	}
	...
	pub fn add_to_index(
		...
		unsafe {
			try_call!(raw::git_submodule_add_to_index(
				self.raw,
				write_index
			));
		}
...
impl<'cb> SubmoduleUpdateOptions<'cb> {
	...
	unsafe fn raw(&mut self) -> raw::git_submodule_update_options {
		let mut checkout_opts: raw::git_checkout_options =
			mem::zeroed();
		let init_res = raw::git_checkout_init_options(
			&mut checkout_opts,
...
mod tests {
	...
	fn add_a_submodule() {
		...
		let url =
			Url::from_file_path(&repo1.workdir().unwrap()).unwrap();
		let mut s = repo2
	...
	fn clone_submodule() {
		...
		let url1 =
			Url::from_file_path(&repo1.workdir().unwrap()).unwrap();
		let url2 =
			Url::from_file_path(&repo2.workdir().unwrap()).unwrap();
		let mut s1 = parent
	...
	fn repo_init_submodule() {
		...
		let url_child =
			Url::from_file_path(&child.workdir().unwrap()).unwrap();
		let url_parent =
			Url::from_file_path(&parent.workdir().unwrap()).unwrap();
		let mut sub = parent
		...
		let td = TempDir::new().unwrap();
		let new_parent =
			Repository::clone(&url_parent.to_string(), &td).unwrap();
