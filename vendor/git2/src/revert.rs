...
impl<'cb> RevertOptions<'cb> {
	...
	pub fn checkout_builder(
		&mut self,
		cb: CheckoutBuilder<'cb>,
	) -> &mut Self {
	...
	pub fn raw(&mut self) -> raw::git_revert_options {
		unsafe {
			...
			let mut merge_opts: raw::git_merge_options =
				mem::zeroed();
			raw::git_merge_init_options(
				&mut merge_opts,
			...
			let mut revert_opts: raw::git_revert_options =
				mem::zeroed();
			raw::git_revert_options_init(
				&mut revert_opts,
				raw::GIT_REVERT_OPTIONS_VERSION,
			);
