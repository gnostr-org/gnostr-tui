...
impl Cred {
	...
	pub fn ssh_key_from_agent(username: &str) -> Result<Cred, Error> {
		...
		unsafe {
			try_call!(raw::git_cred_ssh_key_from_agent(
				&mut out, username
			));
			Ok(Binding::from_raw(out))
		}
...
impl CredentialHelper {
	...
	fn config_use_http_path(&mut self, config: &Config) {
		...
		if let Some(value) =
			config.get_bool(&self.exact_key("useHttpPath")).ok()
		{
			use_http_path = value;
			...
			.and_then(|key| config.get_bool(&key).ok())
		{
			...
			config.get_bool("credential.useHttpPath").ok()
		{
			use_http_path = value;
		}
	...
	fn url_key(&self, name: &str) -> Option<String> {
		match (&self.host, &self.protocol) {
			(&Some(ref host), &Some(ref protocol)) => Some(format!(
				"credential.{}://{}.{}",
				protocol, host, name
			)),
			_ => None,
...
",
		...
		let paths = env::split_paths(&paths).chain(
			path.parent().map(|p| p.to_path_buf()).into_iter(),
		);
...
",
	...
	fn credential_helper9() {
		...
		let (u, p) =
			CredentialHelper::new("https://example.com:3000/foo/bar")
				.config(&cfg)
				.execute()
				.unwrap();
		assert_eq!(u, "a");
