...
use crate::{
	panic, raw, Error, IndexAddOption, IndexTime, Oid, Repository,
	Tree,
};
...
impl Index {
	...
	pub fn add_path(&mut self, path: &Path) -> Result<(), Error> {
		...
		unsafe {
			try_call!(raw::git_index_add_bypath(
				self.raw, posix_path
			));
			Ok(())
		}
	...
	{
		let (_a, _b, raw_strarray) =
			crate::util::iter2cstrs_paths(pathspecs)?;
		let ptr = cb.as_mut();
		let callback = ptr.as_ref().map(|_| {
			index_matched_path_cb as extern "C" fn(_, _, _) -> _
		});
		unsafe {
			try_call!(raw::git_index_add_all(
				...
				flag.bits() as c_uint,
				callback,
				ptr.map(|p| p as *mut _).unwrap_or(ptr::null_mut())
					as *mut c_void
			));
		}
		Ok(())
	...
	pub fn get(&self, n: usize) -> Option<IndexEntry> {
		unsafe {
			let ptr =
				raw::git_index_get_byindex(self.raw, n as size_t);
			if ptr.is_null() {
				None
			} else {
	...
	pub fn get_path(
		...
		unsafe {
			let ptr = call!(raw::git_index_get_bypath(
				self.raw,
				path,
				stage as c_int
			));
	...
	pub fn path(&self) -> Option<&Path> {
		unsafe {
			crate::opt_bytes(self, raw::git_index_path(&*self.raw))
				.map(util::bytes2path)
		}
	}
	...
	pub fn read_tree(
		...
		unsafe {
			try_call!(raw::git_index_read_tree(
				self.raw,
				&*tree.raw()
			));
		}
	...
	pub fn write_tree_to(
		...
		unsafe {
			try_call!(raw::git_index_write_tree_to(
				&mut raw,
				self.raw,
				repo.raw()
			));
	...
	pub fn find_prefix<T: IntoCString>(
		&self,
		prefix: T,
	) -> Result<usize, Error> {
...
impl<'index> Binding for IndexConflicts<'index> {
	...
	unsafe fn from_raw(
		raw: *mut raw::git_index_conflict_iterator,
	) -> IndexConflicts<'index> {
		IndexConflicts {
			conflict_iter: raw,
...
extern "C" fn index_matched_path_cb(
	...
	unsafe {
		...
		let matched_pathspec =
			CStr::from_ptr(matched_pathspec).to_bytes();

		panic::wrap(|| {
			let payload = payload as *mut &mut IndexMatchedPath<'_>;
			(*payload)(util::bytes2path(path), matched_pathspec)
				as c_int
		})
		.unwrap_or(-1)
	}
...
impl<'index> Drop for IndexConflicts<'index> {
	fn drop(&mut self) {
		unsafe {
			raw::git_index_conflict_iterator_free(self.conflict_iter)
		}
	}
}
...
impl Binding for IndexEntry {
	...
	unsafe fn from_raw(raw: raw::git_index_entry) -> IndexEntry {
		...
		// nul-terminated.
		let mut pathlen =
			(flags & raw::GIT_INDEX_ENTRY_NAMEMASK) as usize;
...
mod tests {
	...
	use crate::{
		ErrorCode, Index, IndexEntry, IndexTime, Oid, Repository,
		ResetType,
	};
