...
use crate::{
	raw, signature, Buf, Error, IntoCString, Mailmap, Object, Oid,
	Signature, Time, Tree,
};
...
impl<'repo> Commit<'repo> {
	...
	pub fn tree_id(&self) -> Oid {
		unsafe {
			Binding::from_raw(raw::git_commit_tree_id(&*self.raw))
		}
	}
	...
	pub fn message_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_commit_message(&*self.raw),
			)
			.unwrap()
		}
	}
	...
	pub fn message_encoding(&self) -> Option<&str> {
		let bytes = unsafe {
			crate::opt_bytes(
				self,
				raw::git_commit_message_encoding(&*self.raw),
			)
		};
	...
	pub fn message_raw_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_commit_message_raw(&*self.raw),
			)
			.unwrap()
		}
	}
	...
	pub fn header_field_bytes<T: IntoCString>(
		&self,
		field: T,
	) -> Result<Buf, Error> {
		let buf = Buf::new();
	...
	pub fn raw_header_bytes(&self) -> &[u8] {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_commit_raw_header(&*self.raw),
			)
			.unwrap()
		}
	}
	...
	pub fn summary_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(self, raw::git_commit_summary(self.raw))
		}
	}
	...
	pub fn body_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(self, raw::git_commit_body(self.raw))
		}
	}
	...
	pub fn parent_id(&self, i: usize) -> Result<Oid, Error> {
		unsafe {
			let id = raw::git_commit_parent_id(
				self.raw,
				i as libc::c_uint,
			);
			if id.is_null() {
...
mod tests {
	...
	fn smoke() {
		...
		let tree_header_bytes =
			commit.header_field_bytes("tree").unwrap();
		assert_eq!(
			crate::Oid::from_str(tree_header_bytes.as_str().unwrap())
				.unwrap(),
			commit.tree_id()
		);
