...
impl Config {
	...
	pub fn remove_multivar(
		...
		unsafe {
			try_call!(raw::git_config_delete_multivar(
				self.raw, name, regexp
			));
		}
		Ok(())
	}
	...
	pub fn get_bool(&self, name: &str) -> Result<bool, Error> {
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_bool(
				&mut out, &*self.raw, name
			));
		}
		Ok(out != 0)
	}
	...
	pub fn get_i32(&self, name: &str) -> Result<i32, Error> {
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_int32(
				&mut out, &*self.raw, name
			));
		}
		Ok(out)
	...
	pub fn get_i64(&self, name: &str) -> Result<i64, Error> {
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_int64(
				&mut out, &*self.raw, name
			));
		}
		Ok(out)
	...
	pub fn get_str(&self, name: &str) -> Result<&str, Error> {
		str::from_utf8(self.get_bytes(name)?).map_err(|_| {
	...
	pub fn get_bytes(&self, name: &str) -> Result<&[u8], Error> {
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_string(
	...
	pub fn get_string(&self, name: &str) -> Result<String, Error> {
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_string_buf(
				ret.raw(),
				self.raw,
				name
			));
	...
	pub fn get_path(&self, name: &str) -> Result<PathBuf, Error> {
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_path(
				ret.raw(),
				self.raw,
				name
			));
	...
	pub fn get_entry(
		&self,
		name: &str,
	) -> Result<ConfigEntry<'_>, Error> {
		let mut ret = ptr::null_mut();
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_get_entry(
				&mut ret, self.raw, name
			));
	...
	pub fn entries(
		...
		unsafe {
			match glob {
				Some(s) => {
					let s = CString::new(s)?;
					try_call!(raw::git_config_iterator_glob_new(
						&mut ret, &*self.raw, s
					));
				}
				None => {
					try_call!(raw::git_config_iterator_new(
						&mut ret, &*self.raw
					));
				}
			}
	...
	pub fn open_global(&mut self) -> Result<Config, Error> {
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_config_open_global(
				&mut raw, self.raw
			));
			Ok(Binding::from_raw(raw))
		}
	...
	pub fn open_level(
		...
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_config_open_level(
				&mut raw, &*self.raw, level
			));
			Ok(Binding::from_raw(raw))
		}
	}
	...
	pub fn set_bool(
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_set_bool(
				self.raw, name, value
			));
		}
		Ok(())
	...
	pub fn set_i32(
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_set_int32(
				self.raw, name, value
			));
		}
		Ok(())
	...
	pub fn set_i64(
		...
		let name = CString::new(name)?;
		unsafe {
			try_call!(raw::git_config_set_int64(
				self.raw, name, value
			));
		}
		Ok(())
	...
	pub fn set_multivar(
		...
		unsafe {
			try_call!(raw::git_config_set_multivar(
				self.raw, name, regexp, value
			));
		}
		Ok(())
	}
	...
	pub fn set_str(
		...
		let value = CString::new(value)?;
		unsafe {
			try_call!(raw::git_config_set_string(
				self.raw, name, value
			));
		}
		Ok(())
...
impl<'cfg> ConfigEntries<'cfg> {
	...
	pub fn next(
		&mut self,
	) -> Option<Result<&ConfigEntry<'cfg>, Error>> {
		let mut raw = ptr::null_mut();
	...
	pub fn for_each<F: FnMut(&ConfigEntry<'cfg>)>(
		mut self,
		mut f: F,
	) -> Result<(), Error> {
...
mod tests {
	...
	fn multivar() {
		...
		cfg.entries(Some("foo.bar"))
			.unwrap()
			.for_each(|entry| {
				entries.push(entry.value().unwrap().to_string())
			})
			.unwrap();
		entries.sort();
		...
		cfg.multivar("foo.bar", None)
			.unwrap()
			.for_each(|entry| {
				multivals.push(entry.value().unwrap().to_string())
			})
			.unwrap();
		multivals.sort();
		...
		cfg.multivar("foo.bar", Some("qu.*x"))
			.unwrap()
			.for_each(|entry| {
				quxish.push(entry.value().unwrap().to_string())
			})
			.unwrap();
		quxish.sort();
	...
	fn parse() {
		...
		assert_eq!(
			Config::parse_i64("1G").unwrap(),
			1024 * 1024 * 1024
		);
		assert_eq!(
			Config::parse_i64("100G").unwrap(),
			100 * 1024 * 1024 * 1024
		);
	}
}
