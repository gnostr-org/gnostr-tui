...
use crate::{
	raw, Blob, Buf, Diff, DiffDelta, DiffHunk, DiffLine, DiffOptions,
	Error,
};
...
impl<'buffers> Patch<'buffers> {
	...
	pub fn from_diff(
		...
		unsafe {
			try_call!(raw::git_patch_from_diff(
				&mut ret,
				diff.raw(),
				idx
			));
	...
	pub fn delta(&self) -> DiffDelta<'buffers> {
		unsafe {
			Binding::from_raw(
				raw::git_patch_get_delta(self.raw) as *mut _
			)
		}
	}
	...
	pub fn num_lines_in_hunk(
		&self,
		hunk_idx: usize,
	) -> Result<usize, Error> {
		unsafe {
			Ok(try_call!(raw::git_patch_num_lines_in_hunk(
				self.raw, hunk_idx
			)) as usize)
		}
	}
