...
impl<'repo> TreeBuilder<'repo> {
	...
	pub fn get<P>(
		&self,
		filename: P,
	) -> Result<Option<TreeEntry<'_>>, Error>
	where
		P: IntoCString,
	{
		...
		unsafe {
			let ret =
				raw::git_treebuilder_get(self.raw, filename.as_ptr());
			if ret.is_null() {
				Ok(None)
			} else {
	...
	pub fn remove<P: IntoCString>(
		&mut self,
		filename: P,
	) -> Result<(), Error> {
		let filename = filename.into_c_string()?;
		unsafe {
			try_call!(raw::git_treebuilder_remove(
				self.raw, filename
			));
		}
		Ok(())
	...
	pub fn write(&self) -> Result<Oid, Error> {
		...
		unsafe {
			try_call!(raw::git_treebuilder_write(
				&mut raw,
				self.raw()
			));
