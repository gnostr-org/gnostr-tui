...
mod impls {
	...
	use crate::{
		raw, BranchType, ConfigLevel, Direction, ObjectType,
		ResetType,
	};
	use crate::{
		AutotagOption, DiffFormat, FetchPrune, FileFavor,
		...
		fn convert(&self) -> raw::git_diff_format_t {
			match *self {
				...
				DiffFormat::PatchHeader => {
					raw::GIT_DIFF_FORMAT_PATCH_HEADER
				}
				DiffFormat::Raw => raw::GIT_DIFF_FORMAT_RAW,
				DiffFormat::NameOnly => {
					raw::GIT_DIFF_FORMAT_NAME_ONLY
				}
				DiffFormat::NameStatus => {
					raw::GIT_DIFF_FORMAT_NAME_STATUS
				}
	...
	impl Convert<raw::git_submodule_update_t> for SubmoduleUpdate {
		fn convert(&self) -> raw::git_submodule_update_t {
			match *self {
				...
				SubmoduleUpdate::Merge => {
					raw::GIT_SUBMODULE_UPDATE_MERGE
				}
				SubmoduleUpdate::None => {
					raw::GIT_SUBMODULE_UPDATE_NONE
				}
	...
	impl Convert<raw::git_remote_autotag_option_t> for AutotagOption {
		fn convert(&self) -> raw::git_remote_autotag_option_t {
			match *self {
				...
				AutotagOption::None => {
					raw::GIT_REMOTE_DOWNLOAD_TAGS_NONE
				}
				AutotagOption::Auto => {
					raw::GIT_REMOTE_DOWNLOAD_TAGS_AUTO
				}
				AutotagOption::All => {
					raw::GIT_REMOTE_DOWNLOAD_TAGS_ALL
				}
			}
		}
	}
		...
		fn convert(&self) -> raw::git_fetch_prune_t {
			match *self {
				FetchPrune::Unspecified => {
					raw::GIT_FETCH_PRUNE_UNSPECIFIED
				}
