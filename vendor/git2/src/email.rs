...
use crate::{
	raw, Buf, Commit, DiffFindOptions, DiffOptions, Error,
	IntoCString,
};
...
impl EmailCreateOptions {
	...
	fn flag(
		&mut self,
		opt: raw::git_email_create_flags_t,
		val: bool,
	) -> &mut Self {
		let opt = opt as u32;
		if val {
	...
	pub fn subject_prefix<T: IntoCString>(
		&mut self,
		t: T,
	) -> &mut Self {
	...
	unsafe fn raw(&mut self) -> *const raw::git_email_create_options {
		...
		self.raw.diff_find_opts =
			ptr::read(self.diff_find_options.raw());
		&self.raw as *const _
	}
}
