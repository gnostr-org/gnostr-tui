...
impl<'repo, 'cb, C: FnMut(Oid) -> bool>
	RevwalkWithHideCb<'repo, 'cb, C>
{
...
impl<'repo> Revwalk<'repo> {
	...
	pub fn simplify_first_parent(&mut self) -> Result<(), Error> {
		unsafe {
			try_call!(raw::git_revwalk_simplify_first_parent(
				self.raw
			));
		}
		Ok(())
...
impl<'repo> Iterator for Revwalk<'repo> {
	...
	fn next(&mut self) -> Option<Result<Oid, Error>> {
		...
		unsafe {
			try_call_iter!(raw::git_revwalk_next(
				&mut out,
				self.raw()
			));
...
impl<'repo, 'cb, C: FnMut(Oid) -> bool> Iterator
	for RevwalkWithHideCb<'repo, 'cb, C>
{
	type Item = Result<Oid, Error>;
	fn next(&mut self) -> Option<Result<Oid, Error>> {
...
mod tests {
	...
	fn smoke() {
		...
		let oids: Vec<crate::Oid> =
			walk.by_ref().collect::<Result<Vec<_>, _>>().unwrap();
