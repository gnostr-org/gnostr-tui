...
impl Worktree {
	...
	pub fn open_from_repository(
		repo: &Repository,
	) -> Result<Worktree, Error> {
		let mut raw = ptr::null_mut();
		unsafe {
			try_call!(raw::git_worktree_open_from_repository(
				&mut raw,
				repo.raw()
			));
	...
	pub fn path(&self) -> &Path {
		unsafe {
			util::bytes2path(
				crate::opt_bytes(
					self,
					raw::git_worktree_path(self.raw),
				)
				.unwrap(),
			)
		}
	...
	pub fn is_locked(&self) -> Result<WorktreeLockStatus, Error> {
		let buf = Buf::new();
		unsafe {
			match try_call!(raw::git_worktree_is_locked(
				buf.raw(),
				self.raw
			)) {
	...
	pub fn prune(
		...
		unsafe {
			try_call!(raw::git_worktree_prune(
				self.raw,
				opts.map(|o| o.raw())
			));
		}
...
impl<'a> WorktreeAddOptions<'a> {
	...
	pub fn new() -> WorktreeAddOptions<'a> {
		unsafe {
			...
			assert_eq!(
				raw::git_worktree_add_options_init(
					&mut raw,
...
mod tests {
	...
	fn smoke_add_no_ref() {
		...
		let wt = repo
			.worktree("tree-no-ref", &wt_path, Some(&opts))
			.unwrap();
	...
	fn smoke_add_locked() {
		...
		let wt = repo
			.worktree("locked-tree", &wt_path, Some(&opts))
			.unwrap();
