...
use crate::{
	panic, raw, Buf, Delta, DiffFormat, Error, FileMode, Oid,
	Repository,
};
...
type PrintCb<'a> = dyn FnMut(DiffDelta<'_>, Option<DiffHunk<'_>>, DiffLine<'_>) -> bool
	+ 'a;

pub type FileCb<'a> = dyn FnMut(DiffDelta<'_>, f32) -> bool + 'a;
pub type BinaryCb<'a> =
	dyn FnMut(DiffDelta<'_>, DiffBinary<'_>) -> bool + 'a;
pub type HunkCb<'a> =
	dyn FnMut(DiffDelta<'_>, DiffHunk<'_>) -> bool + 'a;
pub type LineCb<'a> = dyn FnMut(DiffDelta<'_>, Option<DiffHunk<'_>>, DiffLine<'_>) -> bool
	+ 'a;
...
impl<'repo> Diff<'repo> {
	...
	pub fn get_delta(&self, i: usize) -> Option<DiffDelta<'_>> {
		unsafe {
			let ptr =
				raw::git_diff_get_delta(&*self.raw, i as size_t);
	...
	#[doc(hidden)]
	#[deprecated(
		note = "refactored to `Email::from_diff` to match upstream"
	)]
	pub fn format_email(
		&mut self,
		patch_no: usize,
		...
		let raw_opts =
			opts.map_or(&mut default.raw, |opts| &mut opts.raw);
		...
		#[allow(deprecated)]
		unsafe {
			try_call!(raw::git_diff_format_email(
				buf.raw(),
				self.raw,
				&*raw_opts
			));
	...
	pub fn patchid(
		&self,
		opts: Option<&mut DiffPatchidOptions>,
	) -> Result<Oid, Error> {
...
pub extern "C" fn file_cb_c(
	...
	unsafe {
		...
		let r = panic::wrap(|| {
			let cbs = data as *mut DiffCallbacks<
				'_,
				'_,
				'_,
				'_,
...
impl<'a> DiffDelta<'a> {
	...
	pub fn old_file(&self) -> DiffFile<'a> {
		unsafe {
			Binding::from_raw(&(*self.raw).old_file as *const _)
		}
	}
	...
	pub fn new_file(&self) -> DiffFile<'a> {
		unsafe {
			Binding::from_raw(&(*self.raw).new_file as *const _)
		}
	}
}
...
impl<'a> DiffFile<'a> {
	...
	pub fn is_not_binary(&self) -> bool {
		unsafe {
			(*self.raw).flags & raw::GIT_DIFF_FLAG_NOT_BINARY as u32
				!= 0
		}
	}
	...
	pub fn is_valid_id(&self) -> bool {
		unsafe {
			(*self.raw).flags & raw::GIT_DIFF_FLAG_VALID_ID as u32
				!= 0
		}
	}
	...
	pub fn mode(&self) -> FileMode {
		match unsafe { (*self.raw).mode.into() } {
			...
			raw::GIT_FILEMODE_BLOB_GROUP_WRITABLE => {
				FileMode::BlobGroupWritable
			}
			raw::GIT_FILEMODE_BLOB_EXECUTABLE => {
...
impl DiffOptions {
	...
	pub fn include_typechange_trees(
		&mut self,
		include: bool,
	) -> &mut DiffOptions {
	...
	pub fn enable_fast_untracked_dirs(
		&mut self,
		enable: bool,
	) -> &mut DiffOptions {
	...
	pub fn include_unreadable_as_untracked(
		...
		self.flag(
			raw::GIT_DIFF_INCLUDE_UNREADABLE_AS_UNTRACKED,
			include,
		)
	}
	...
	pub fn ignore_whitespace_change(
		&mut self,
		ignore: bool,
	) -> &mut DiffOptions {
	...
	pub fn old_prefix<T: IntoCString>(
		&mut self,
		t: T,
	) -> &mut DiffOptions {
	...
	pub fn new_prefix<T: IntoCString>(
		&mut self,
		t: T,
	) -> &mut DiffOptions {
	...
	pub unsafe fn raw(&mut self) -> *const raw::git_diff_options {
		...
		self.raw.pathspec.strings =
			self.pathspec_ptrs.as_ptr() as *mut _;
		&self.raw as *const _
	}
...
impl<'a> DiffLine<'a> {
	...
	pub fn origin_value(&self) -> DiffLineType {
		unsafe {
			Binding::from_raw(
				(*self.raw).origin as raw::git_diff_line_t,
			)
		}
	}
...
impl DiffStats {
	...
	pub fn files_changed(&self) -> usize {
		unsafe {
			raw::git_diff_stats_files_changed(&*self.raw) as usize
		}
	}
...
impl DiffFindOptions {
	...
	pub fn ignore_leading_whitespace(
		...
		self.flag(
			raw::GIT_DIFF_FIND_IGNORE_LEADING_WHITESPACE,
			ignore,
		)
	}
	...
	pub fn break_rewrite_threshold(
		&mut self,
		thresh: u16,
	) -> &mut DiffFindOptions {
	...
	pub unsafe fn raw(
		&mut self,
	) -> *const raw::git_diff_find_options {
		&self.raw
	}
}
...
impl DiffFormatEmailOptions {
	...
	pub fn new() -> Self {
		...
		assert_eq!(
			unsafe {
				raw::git_diff_format_email_options_init(
					&mut opts.raw,
					1,
				)
			},
	...
	pub fn exclude_subject_patch_header(
		&mut self,
		should_exclude: bool,
	) -> &mut Self {
		self.flag(
...
mod tests {
	...
	fn foreach_file_only() {
		...
		let diff =
			t!(repo.diff_tree_to_workdir(None, Some(&mut opts)));
		let mut count = 0;
		let mut result = None;
		t!(diff.foreach(
			&mut |file, _progress| {
				count = count + 1;
				result =
					file.new_file().path().map(ToOwned::to_owned);
				true
			},
			None,
			None,
	...
	fn foreach_file_and_hunk() {
		...
		let mut opts = DiffOptions::new();
		opts.include_untracked(true);
		let diff = t!(repo.diff_tree_to_index(
			None,
			Some(&index),
			Some(&mut opts)
		));
	...
	fn foreach_all_callbacks() {
		...
		let deflated_fib = vec![
			120, 156, 99, 96, 100, 100, 98, 102, 229, 0, 0, 0, 53, 0,
			21,
		];
		let foo_path = Path::new("foo");
		let bin_path = Path::new("bin");
		let (td, repo) = crate::test::repo_init();
		t!(t!(File::create(&td.path().join(foo_path)))
			.write_all(b"bar\n"));
		t!(t!(File::create(&td.path().join(bin_path)))
			.write_all(&fib));
		let mut index = t!(repo.index());
		...
		t!(diff.foreach(
			...
			Some(&mut |_file, binary| {
				bin_content =
					Some(binary.new_file().data().to_owned());
				true
			}),
			Some(&mut |_file, hunk| {
				new_lines = hunk.new_lines();
			...
			Some(&mut |_file, _hunk, line| {
				line_content =
					String::from_utf8(line.content().into()).ok();
				true
			})
		));
	...
	fn format_email_simple() {
		...
		let original_file =
			repo.blob(ORIGINAL_FILE.as_bytes()).unwrap();
		let updated_file =
			repo.blob(UPDATED_FILE.as_bytes()).unwrap();
		...
		let author =
			Signature::new("Techcable", "dummy@dummy.org", &time)
				.unwrap();
		let updated_commit = repo
			...
			.unwrap();
		let updated_commit =
			repo.find_commit(updated_commit).unwrap();
		let mut diff = repo
			.diff_tree_to_tree(
		...
		#[allow(deprecated)]
		let actual_email =
			diff.format_email(1, 1, &updated_commit, None).unwrap();
		...
		let mut remaining_lines =
			actual_email[EXPECTED_EMAIL_START.len()..].lines();
