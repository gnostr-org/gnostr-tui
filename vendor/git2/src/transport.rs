...
pub trait SmartSubtransportStream:
...
type TransportFactory = dyn Fn(&Remote<'_>) -> Result<Transport, Error>
	+ Send
	+ Sync
	+ 'static;
...
pub unsafe fn register<F>(
	prefix: &str,
	factory: F,
) -> Result<(), Error>
where
	F: Fn(&Remote<'_>) -> Result<Transport, Error>
		+ Send
		+ Sync
		+ 'static,
{
...
extern "C" fn subtransport_action(
	...
	panic::wrap(|| unsafe {
		...
		let transport =
			&mut *(raw_transport as *mut RawSmartSubtransport);
		...
		let generate_stream = transport.rpc
			|| action == Service::UploadPackLs
			|| action == Service::ReceivePackLs;
		if generate_stream {
...
// object.
extern "C" fn subtransport_close(
	transport: *mut raw::git_smart_subtransport,
) -> c_int {
	let ret = panic::wrap(|| unsafe {
		let transport =
			&mut *(transport as *mut RawSmartSubtransport);
		transport.obj.close()
	});
	match ret {
		Some(Ok(())) => 0,
...
extern "C" fn stream_read(
	...
	let ret = panic::wrap(|| unsafe {
		let transport =
			&mut *(stream as *mut RawSmartSubtransportStream);
