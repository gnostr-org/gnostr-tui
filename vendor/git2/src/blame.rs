...
impl<'repo> Blame<'repo> {
	...
	pub fn get_index(&self, index: usize) -> Option<BlameHunk<'_>> {
		unsafe {
			let ptr = raw::git_blame_get_hunk_byindex(
				self.raw(),
				index as u32,
			);
			if ptr.is_null() {
	...
	pub fn get_line(&self, lineno: usize) -> Option<BlameHunk<'_>> {
		unsafe {
			let ptr =
				raw::git_blame_get_hunk_byline(self.raw(), lineno);
			if ptr.is_null() {
				None
			} else {
				Some(BlameHunk::from_raw_const(ptr))
...
impl<'blame> BlameHunk<'blame> {
	...
	pub fn final_signature(&self) -> Signature<'_> {
		unsafe {
			signature::from_raw_const(
				self,
				(*self.raw).final_signature,
			)
		}
	}
	...
	pub fn path(&self) -> Option<&Path> {
		unsafe {
			if let Some(bytes) =
				crate::opt_bytes(self, (*self.raw).orig_path)
			{
...
impl BlameOptions {
	...
	pub fn track_copies_same_commit_moves(
		&mut self,
		opt: bool,
	) -> &mut BlameOptions {
	...
	/// Track lines that have been copied from another file that exists
	/// in any commit.
	pub fn track_copies_any_commit_copies(
		&mut self,
		opt: bool,
	) -> &mut BlameOptions {
...
mod tests {
	...
	fn smoke() {
		...
		let blame =
			repo.blame_file(Path::new("foo/bar"), None).unwrap();
