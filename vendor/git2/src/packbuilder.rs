...
pub type ProgressCb<'a> =
	dyn FnMut(PackBuilderStage, u32, u32) -> bool + 'a;
...
impl<'repo> PackBuilder<'repo> {
	...
	pub fn insert_object(
		...
		unsafe {
			try_call!(raw::git_packbuilder_insert(
				self.raw,
				id.raw(),
				name
			));
	...
	pub fn insert_tree(&mut self, id: Oid) -> Result<(), Error> {
		unsafe {
			try_call!(raw::git_packbuilder_insert_tree(
				self.raw,
				id.raw()
			));
		}
	...
	pub fn insert_commit(&mut self, id: Oid) -> Result<(), Error> {
		unsafe {
			try_call!(raw::git_packbuilder_insert_commit(
				self.raw,
				id.raw()
			));
		}
	...
	pub fn insert_walk(
		...
		unsafe {
			try_call!(raw::git_packbuilder_insert_walk(
				self.raw,
				walk.raw()
			));
		}
	...
	pub fn insert_recursive(
		...
		let name = crate::opt_cstr(name)?;
		unsafe {
			try_call!(raw::git_packbuilder_insert_recur(
				self.raw,
				id.raw(),
				name
			));
	...
	pub fn write_buf(&mut self, buf: &mut Buf) -> Result<(), Error> {
		unsafe {
			try_call!(raw::git_packbuilder_write_buf(
				buf.raw(),
				self.raw
			));
		}
		...
		F: FnMut(&[u8]) -> bool,
	{
		...
		let foreach: raw::git_packbuilder_foreach_cb =
			Some(foreach_c);
		unsafe {
	...
	{
		let mut progress =
			Box::new(Box::new(progress) as Box<ProgressCb<'_>>);
		let ptr = &mut *progress as *mut _;
		let progress_c: raw::git_packbuilder_progress =
			Some(progress_c);
		unsafe {
	...
	pub fn hash(&self) -> Option<Oid> {
		if self.object_count() == 0 {
			unsafe {
				Some(Binding::from_raw(raw::git_packbuilder_hash(
					self.raw,
				)))
			}
		} else {
	...
	pub fn name_bytes(&self) -> Option<&[u8]> {
		unsafe {
			crate::opt_bytes(
				self,
				raw::git_packbuilder_name(self.raw),
			)
		}
	}
}
...
impl<'repo> Drop for PackBuilder<'repo> {
	fn drop(&mut self) {
		unsafe {
			raw::git_packbuilder_set_callbacks(
				self.raw,
				None,
				ptr::null_mut(),
			);
...
impl Binding for PackBuilderStage {
	...
	unsafe fn from_raw(
		raw: raw::git_packbuilder_stage_t,
	) -> PackBuilderStage {
		match raw {
			raw::GIT_PACKBUILDER_ADDING_OBJECTS => {
				PackBuilderStage::AddingObjects
			}
			raw::GIT_PACKBUILDER_DELTAFICATION => {
...
extern "C" fn foreach_c(
	...
	unsafe {
		let buf =
			slice::from_raw_parts(buf as *const u8, size as usize);
	...
	fn empty_pack_header() -> Vec<u8> {
		pack_header(0)
			...
			.chain(&[
				...
				0xd8, 0xa8, 0xea, 0xb5,
				0x10, // | SHA-1 of the zero
				0xad, 0x6a, 0xc7, 0x5c,
				0x82, // | object pack header
